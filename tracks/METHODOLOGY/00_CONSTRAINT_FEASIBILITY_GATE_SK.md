...
## 9. Povinný register funkcie, rozsahy a topológia smrti

Každá povinná neznáma funkcia má na najnižšej route úrovni jeden živý
`00_STATUS.md` alebo work plan s týmto minimom:
...
Nadradený plán vedie iba ID, stav a odkaz. Po prijatí rozsahu sa celý
rozsah prenáša do potomkov; nesmie sa nahradiť jedným pohodlným bodom.

### 9.1 Rozhodovanie vetiev

- `AND` uzol predstavuje spoločne povinné mechanizmy. Jeho prípustná
  množina je prienik pullbackov všetkých detí a zomiera iba po certifikáte
  prázdnosti tohto spoločného prieniku.
- `OR` uzol predstavuje alternatívne koľaje. Zomiera iba ak je rozdelenie
  alternatív preukázane úplné a každá alternatíva je
  `EMPTY_SCOPE_CERTIFIED` v rovnakom scope.
- neotestovaná alebo iba nenájdená alternatíva udržiava rodiča
  `LIVE / WAITING`; nedáva PASS, ale blokuje falošnú smrť.
- teória môže dostať `THEORY_PHYSICALLY_DEAD_IN_SCOPE` až keď je
  `A_theory` certifikovane prázdna a všetky top-level alternatívne koľaje v
  úplnom rozdelení zomreli. Dovtedy je najprísnejší globálny stav
  `GLOBAL_FEASIBILITY_INCOMPLETE`.

### 9.2 Hierarchia rozhodnutí

```text
NOT_MAPPED
  -> RANGE_CONDITIONAL_OPEN
  -> RANGE_EXISTENCE_PASS          (existuje aspoň jeden spoločný rozsah)
  -> NONEMPTY_WITNESS              (existuje explicitná funkcia/objekt)
  -> príslušné G-gate testy

alebo

NOT_MAPPED
  -> EMPTY_SCOPE_CERTIFIED         (iba presný scope)
  -> parent AND/OR propagation
  -> TRACK_PHYSICALLY_DEAD_IN_SCOPE
  -> THEORY_PHYSICALLY_DEAD_IN_SCOPE iba po globálnom certifikáte
```

`RANGE_EXISTENCE_PASS` je výsledok existencie rozsahu, nie dôkaz úplnej
fyziky. `EMPTY_SCOPE_CERTIFIED` je výsledok neexistencie presného priestoru,
nie automaticky smrť celej koľaje alebo teórie.
