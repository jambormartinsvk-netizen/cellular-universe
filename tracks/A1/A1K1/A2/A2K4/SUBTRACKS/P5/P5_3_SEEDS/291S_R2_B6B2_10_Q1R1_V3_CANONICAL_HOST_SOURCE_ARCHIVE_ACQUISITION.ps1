...
$ExpectedPreregSha256 = 'A51FB8AD984BECAA1F1FD6B98B8E2415E2287B4C75F660A28E2B8E30AD10F469'
$ObservedLocationJournalSha256 = '307D9769C7690B4EF70F2A97E2C7B1D84C2A9FAC35121CE9F827205FF054DD85'
$ExpectedParentTerminalJournalSha256 = '06F0BB6124CCFDEF4AC7D291C27E3CF39B00B821175A257C30393E05B3F4EE99'
$ExpectedAuthorizationTaskPrefix = 'TASK_ID: A2K4-B6B2-10-H-RDIV-C01-RW1-Q1R1-V3-O3-EXACT-TAIL-SOURCE-ARCHIVE-ACCESS-AUTHORIZATION-'
$ExpectedSourceAccessAuthorizationMarker = 'SOURCE_ACCESS_AUTHORIZED: true_Q1R1_V3_O3_EXACT_TAIL_SRC_ARCHIVE_GET'
$SourceUrl = 'https://arxiv.org/src/2307.12080v2'
$UserAgent = 'Teoria-Q1R1-V3-SourceAudit/1.0'
$MinHttpBytes = 10000L
...
namespace Teoria.Q1R1V3
{
    public sealed class ExactTailReadStream : Stream
    {
        private readonly Stream inner;
        private readonly long exactTailStart;
        public ExactTailReadStream(Stream inner, long exactTailStart)
        {
            this.inner = inner ?? throw new ArgumentNullException(nameof(inner));
            if (!inner.CanSeek) throw new ArgumentException("Stream must be seekable.", nameof(inner));
            if (exactTailStart < inner.Position || exactTailStart > inner.Length)
                throw new ArgumentOutOfRangeException(nameof(exactTailStart));
            this.exactTailStart = exactTailStart;
        }
        private int GetReadCount(int requested)
        {
            if (requested <= 0) return 0;
            long remainingPrefix = exactTailStart - inner.Position;
            if (remainingPrefix <= 0) return Math.Min(1, requested);
            return (int)Math.Min((long)requested, remainingPrefix);
        }
        ...
        public override int Read(byte[] buffer, int offset, int count) => inner.Read(buffer, offset, GetReadCount(count));
        public override int Read(Span<byte> buffer) => inner.Read(buffer.Slice(0, GetReadCount(buffer.Length)));
...
function Assert-AuthorizationLedgerMarkers {
    param(
        [Parameter(Mandatory = $true)][string]$LedgerText,
        [Parameter(Mandatory = $true)][string]$ScriptSha256
    )
    foreach ($marker in @(
        $ExpectedAuthorizationTaskPrefix,
        'TRANSPORT_INPUT_DELTA_ID: Q1R1-V3-O2-OBSERVED-SRC-DELTA-001',
        'RUNTIME_DELTA_ID: Q1R1-V3-O3-EXACT-TAIL-DEFLATE-DELTA-001',
        "PARENT_TERMINAL_JOURNAL_SHA256: $ExpectedParentTerminalJournalSha256",
        "FROZEN_PREREG291_SHA256: $ExpectedPreregSha256",
        "FROZEN_291S_R2_SHA256: $ScriptSha256",
        $ExpectedSourceAccessAuthorizationMarker
    )) {
...
function Get-LocationEvidence {
    param(
        [AllowEmptyCollection()][string[]]$Values,
        [Parameter(Mandatory = $true)][System.Uri]$RequestUri
    )

    $orderedValues = [System.Collections.Generic.List[string]]::new()
    if ($null -ne $Values) {
        foreach ($value in $Values) {
            if ($null -eq $value) { Throw-Guard 'LOCATION_NULL_VALUE' }
            $orderedValues.Add([string]$value)
        }
    }
    ...
    try {
        [byte[]]$prefix = $utf8.GetBytes("LOCATION_VALUES_V1`0")
        $frame.Write($prefix, 0, $prefix.Length)
        Write-U32BigEndian $frame ([uint32]$orderedValues.Count)
        foreach ($value in $orderedValues) {
            [byte[]]$valueBytes = $utf8.GetBytes($value)
            Write-U32BigEndian $frame ([uint32]$valueBytes.Length)
            $frame.Write($valueBytes, 0, $valueBytes.Length)
        }
        $digest = Get-Sha256Hex $frame.ToArray()
    }
    finally { $frame.Dispose() }

    $evidence = [ordered]@{
        ValueCount = $orderedValues.Count
        Present = ($orderedValues.Count -gt 0)
        Class = 'ABSENT'
        ValuesSha256 = $digest
        Normalized = 'ABSENT'
    }
    if ($orderedValues.Count -eq 0) { return [pscustomobject]$evidence }
    if ($orderedValues.Count -ne 1) {
        $evidence.Class = 'OTHER_OR_INVALID'
        return [pscustomobject]$evidence
    }

    $raw = $orderedValues[0]
    [byte[]]$rawBytes = $utf8.GetBytes($raw)
    if ($rawBytes.Length -gt 2048 -or $raw -cne $raw.Trim() -or $raw -match '[\x00-\x1F\x7F]') {
        $evidence.Class = 'OTHER_OR_INVALID'
        return [pscustomobject]$evidence
    }

    try { $resolved = [System.Uri]::new($RequestUri, $raw) }
    catch {
        $evidence.Class = 'OTHER_OR_INVALID'
        return [pscustomobject]$evidence
    }
    if (-not $resolved.IsAbsoluteUri -or
        $resolved.Scheme -cne 'https' -or
        -not $resolved.IsDefaultPort -or
        $resolved.Port -ne 443 -or
        $resolved.UserInfo.Length -ne 0 -or
        $resolved.Query.Length -ne 0 -or
        $resolved.Fragment.Length -ne 0) {
        $evidence.Class = 'OTHER_OR_INVALID'
        return [pscustomobject]$evidence
    }

    $resolvedIdnHost = $resolved.IdnHost.ToLowerInvariant()
    if ($resolvedIdnHost -ceq 'arxiv.org') { $evidence.Class = 'SAME_ORIGIN_HTTPS' }
    elseif ($resolvedIdnHost -ceq 'export.arxiv.org') { $evidence.Class = 'ALLOWED_ARXIV_HOST_HTTPS' }
    else {
        $evidence.Class = 'OTHER_OR_INVALID'
        return [pscustomobject]$evidence
    }

    $components = [System.UriComponents]([int][System.UriComponents]::SchemeAndServer -bor [int][System.UriComponents]::Path)
    $normalized = $resolved.GetComponents($components, [System.UriFormat]::UriEscaped)
    [byte[]]$normalizedBytes = $utf8.GetBytes($normalized)
    if ($normalizedBytes.Length -gt 2048 -or $normalized -match '[\x00-\x1F\x7F]') {
        $evidence.Class = 'OTHER_OR_INVALID'
        return [pscustomobject]$evidence
    }
...
function Get-U16LE {
    ...
    return [Teoria.Q1R1V3.LittleEndian]::ReadUInt16($Bytes, $Offset)
}

function Get-U32LE {
    param([byte[]]$Bytes, [int]$Offset)
    return [Teoria.Q1R1V3.LittleEndian]::ReadUInt32($Bytes, $Offset)
}
...
function Expand-SingleGzipMember {
    ...
    if (($flags -band 0x02) -ne 0) {
        ...
        $expectedHeaderCrc = [uint16]([Teoria.Q1R1V3.Crc32]::Compute($headerBytes) -band 0xFFFF)
    ...
    $exactTailStart = [Math]::Max([long]$index, [long]$Bytes.Length - 8L - 4096L)
    $bounded = [Teoria.Q1R1V3.ExactTailReadStream]::new($input, $exactTailStart)
    ...
    try {
        ...
        $actualCrc = [Teoria.Q1R1V3.Crc32]::Compute($expanded)
...
function New-FhcrcGzip {
    ...
    $crc16 = [uint16]([Teoria.Q1R1V3.Crc32]::Compute($header) -band 0xFFFF)
...
function Invoke-SelfTest {
    ...
    Assert-Pass 'OBSERVED_SRC_TRANSPORT_BINDING' {
        if ($SourceUrl -cne 'https://arxiv.org/src/2307.12080v2' -or
            $ObservedLocationJournalSha256 -cne '307D9769C7690B4EF70F2A97E2C7B1D84C2A9FAC35121CE9F827205FF054DD85' -or
            $ExpectedParentTerminalJournalSha256 -cne '06F0BB6124CCFDEF4AC7D291C27E3CF39B00B821175A257C30393E05B3F4EE99') {
            Throw-Guard 'SELFTEST_OBSERVED_SRC_TRANSPORT_BINDING_MISMATCH'
        }
    }
    Assert-Pass 'AUTHORIZATION_MARKER_CONTRACT_EXACT_ACCEPTS_AND_TYPO_REJECTS' {
        $syntheticScriptSha = 'A' * 64
        $syntheticLedger = @(
            "$ExpectedAuthorizationTaskPrefix`20260729-SYNTHETIC",
            'TRANSPORT_INPUT_DELTA_ID: Q1R1-V3-O2-OBSERVED-SRC-DELTA-001',
            'RUNTIME_DELTA_ID: Q1R1-V3-O3-EXACT-TAIL-DEFLATE-DELTA-001',
            "PARENT_TERMINAL_JOURNAL_SHA256: $ExpectedParentTerminalJournalSha256",
            "FROZEN_PREREG291_SHA256: $ExpectedPreregSha256",
            "FROZEN_291S_R2_SHA256: $syntheticScriptSha",
            $ExpectedSourceAccessAuthorizationMarker
        ) -join "`n"
        Assert-AuthorizationLedgerMarkers -LedgerText $syntheticLedger -ScriptSha256 $syntheticScriptSha

        $typoLedger = $syntheticLedger.Replace(
            $ExpectedSourceAccessAuthorizationMarker,
            'SOURCE_ACCESS_AUTHORIZED: true_Q1R1_V3_O3_exact_observed-src_archive_GET'
        )
        try {
            Assert-AuthorizationLedgerMarkers -LedgerText $typoLedger -ScriptSha256 $syntheticScriptSha
            Throw-Guard 'SELFTEST_AUTHORIZATION_MARKER_TYPO_DID_NOT_REJECT'
        }
        catch {
            if ($_.Exception.Message -notlike '*AUTHORIZATION_MARKER_MISSING*') { throw }
        }
    }
    $requestUri = [System.Uri]::new($SourceUrl)
    Assert-Pass 'LOCATION_ZERO_VALUES_EXACT_DIGEST' {
        $evidence = Get-LocationEvidence -Values ([string[]]@()) -RequestUri $requestUri
        if ($evidence.ValueCount -ne 0 -or $evidence.Present -or $evidence.Class -cne 'ABSENT' -or
            $evidence.ValuesSha256 -cne 'AE1DE26F5B7A315B40493AAD32572AD5D2E6B50A336A755133A61F207ABB10A7' -or
            $evidence.Normalized -cne 'ABSENT') { Throw-Guard 'SELFTEST_LOCATION_ZERO_MISMATCH' }
    }
    Assert-Pass 'LOCATION_RELATIVE_SAME_ORIGIN_EXACT_DIGEST' {
        $evidence = Get-LocationEvidence -Values ([string[]]@('/e-print/2307.12080v2')) -RequestUri $requestUri
        if ($evidence.ValueCount -ne 1 -or -not $evidence.Present -or $evidence.Class -cne 'SAME_ORIGIN_HTTPS' -or
            $evidence.ValuesSha256 -cne '44DA81B7B31C39D27BA67B14627669C508BC68082562D85CCFB7E83FCEC21DFE' -or
            $evidence.Normalized -cne 'https://arxiv.org/e-print/2307.12080v2') {
            Throw-Guard 'SELFTEST_LOCATION_RELATIVE_MISMATCH'
        }
    }
    Assert-Pass 'LOCATION_ABSOLUTE_ALLOWED_EXPORT_HOST_EXACT_DIGEST' {
        $evidence = Get-LocationEvidence -Values ([string[]]@('https://export.arxiv.org/e-print/2307.12080v2')) -RequestUri $requestUri
        if ($evidence.Class -cne 'ALLOWED_ARXIV_HOST_HTTPS' -or
            $evidence.ValuesSha256 -cne '048F0E7C68C62BAE7C83E367161A2D654F26A64D68CDA4EF00B5B7FC15C13E73' -or
            $evidence.Normalized -cne 'https://export.arxiv.org/e-print/2307.12080v2') {
            Throw-Guard 'SELFTEST_LOCATION_EXPORT_MISMATCH'
        }
    }
    Assert-Pass 'LOCATION_MULTIPLE_VALUES_ORDERED_DIGEST_NO_NORMALIZED' {
        $forward = Get-LocationEvidence -Values ([string[]]@('a','bc')) -RequestUri $requestUri
        $reverse = Get-LocationEvidence -Values ([string[]]@('bc','a')) -RequestUri $requestUri
        if ($forward.ValueCount -ne 2 -or $forward.Class -cne 'OTHER_OR_INVALID' -or $forward.Normalized -cne 'ABSENT' -or
            $forward.ValuesSha256 -cne 'FFA38736CCE655A1B4F6A02F511076E66E03276B08E8476B0870071ECC4E75B9' -or
            $reverse.ValuesSha256 -cne '2D025EA6C9180E77F3595C9FF71B7679AC70D9A6D2B40D8E0022C0F53B823F44' -or
            $forward.ValuesSha256 -ceq $reverse.ValuesSha256) {
            Throw-Guard 'SELFTEST_LOCATION_MULTIPLE_FRAMING_MISMATCH'
        }
    }
    $forbiddenLocations = [ordered]@{
        'LOCATION_FORBID_LEADING_WHITESPACE' = ' /e-print/2307.12080v2'
        'LOCATION_FORBID_CONTROL' = "https://arxiv.org/e-print/2307.12080v2$([char]1)"
        'LOCATION_FORBID_OVERLENGTH' = '/' + ('a' * 2049)
        'LOCATION_FORBID_NON_HTTPS' = 'http://arxiv.org/e-print/2307.12080v2'
        'LOCATION_FORBID_NONDEFAULT_PORT' = 'https://arxiv.org:444/e-print/2307.12080v2'
        'LOCATION_FORBID_USERINFO' = 'https://user@arxiv.org/e-print/2307.12080v2'
        'LOCATION_FORBID_QUERY' = 'https://arxiv.org/e-print/2307.12080v2?x=1'
        'LOCATION_FORBID_FRAGMENT' = 'https://arxiv.org/e-print/2307.12080v2#x'
        'LOCATION_FORBID_OTHER_HOST' = 'https://example.org/e-print/2307.12080v2'
    }
    foreach ($fixtureName in $forbiddenLocations.Keys) {
        $fixtureValue = [string]$forbiddenLocations[$fixtureName]
        Assert-Pass $fixtureName {
            $evidence = Get-LocationEvidence -Values ([string[]]@($fixtureValue)) -RequestUri $requestUri
            if ($evidence.Class -cne 'OTHER_OR_INVALID' -or $evidence.Normalized -cne 'ABSENT') {
                Throw-Guard "SELFTEST_$fixtureName`_NOT_REJECTED"
            }
        }
    }
...
'@
    ...
    Assert-Pass 'NEAR_REAL_SIZE_GZIP_VALIDATION_WITHIN_FROZEN_DEADLINE' {
        [byte[]]$largeBlob = [byte[]]::new(2 * 1024 * 1024)
        [System.Random]::new(230712080).NextBytes($largeBlob)
        $largeTar = New-TestTar @(
            [pscustomobject]@{ Name = 'main.tex'; Data = $rootBytes; TypeFlag = [byte][char]'0' },
            [pscustomobject]@{ Name = 'synthetic.bin'; Data = $largeBlob; TypeFlag = [byte][char]'0' }
        )
        $largeGzip = New-TestGzip $largeTar
        $largeValidation = Test-ArchiveBytes $largeGzip
        if ($largeGzip.Length -lt 1500000 -or $largeValidation.ValidationSeconds -gt $ValidationDeadlineSeconds) {
            Throw-Guard 'SELFTEST_NEAR_REAL_SIZE_VALIDATION_DEADLINE_OR_SIZE_MISMATCH'
        }
    }
...
function Invoke-Acquire {
    ...
    $expectedScript = Join-Path $expectedDirectory '291S_R2_B6B2_10_Q1R1_V3_CANONICAL_HOST_SOURCE_ARCHIVE_ACQUISITION.ps1'
    ...
    $preregPath = Join-Path $expectedDirectory '291_B6B2_10_H_RDIV_C01_RW1_Q1R1_V3_CANONICAL_HOST_SOURCE_ARCHIVE_PREREGISTRATION_SK.md'
    $ledgerPath = 'D:\Teoria\tracks\A1\A1K1\A2\A2K4\HISTORY\00_EVENT_LEDGER.md'
    $parentTerminalJournalPath = Join-Path $expectedDirectory '291J_R2_B6B2_10_Q1R1_V3_O2_SRC_SOURCE_ARCHIVE_OPERATION_JOURNAL.txt'
    $journalPath = Join-Path $expectedDirectory '291J_R2_B6B2_10_Q1R1_V3_O3_EXACT_TAIL_SOURCE_ARCHIVE_OPERATION_JOURNAL.txt'
    $tempPath = Join-Path $expectedDirectory '.291A_B6B2_10_Q1R1_V3_SOURCE_ARCHIVE.part'
    $archivePath = Join-Path $expectedDirectory '291A_B6B2_10_Q1R1_ARXIV_2307_12080V2_SOURCE_ARCHIVE.tar.gz'
    $resultPath = Join-Path $expectedDirectory '292_B6B2_10_H_RDIV_C01_RW1_Q1R1_V3_SOURCE_ARCHIVE_ELIGIBILITY_RESULT_SK.md'
    ...
    if (-not [System.IO.File]::Exists($parentTerminalJournalPath)) { Throw-Guard 'PARENT_TERMINAL_JOURNAL_MISSING' }
    $actualParentTerminalJournal = (Get-FileHash -LiteralPath $parentTerminalJournalPath -Algorithm SHA256).Hash
    if ($actualParentTerminalJournal -cne $ExpectedParentTerminalJournalSha256) { Throw-Guard 'PARENT_TERMINAL_JOURNAL_SHA_MISMATCH' }
    ...
    Assert-AuthorizationLedgerMarkers -LedgerText $ledgerText -ScriptSha256 $scriptSha
    ...
    try {
        ...
        Write-JournalLine $journal 'TASK_ID=A2K4-B6B2-10-H-RDIV-C01-RW1-Q1R1-V3-O3-EXACT-TAIL-SOURCE-ARCHIVE-ACCESS'
        Write-JournalLine $journal 'OPERATION_ID=V3-O3'
        ...
        Write-JournalLine $journal 'Q1R1_V2_ARCHIVE_OPERATIONS=1/1_EXHAUSTED'
        Write-JournalLine $journal 'Q1R1_V3_O1=1/1_EXHAUSTED'
        Write-JournalLine $journal 'Q1R1_V3_O2=1/1_EXHAUSTED'
        Write-JournalLine $journal 'CUMULATIVE_SOURCE_OPERATIONS_BEFORE=5'
        Write-JournalLine $journal 'HISTORICAL_PACKAGES_TOTAL_BEFORE=2'
        Write-JournalLine $journal 'CONSECUTIVE_TECHNICAL_FAILURES_BEFORE=6/10'
        Write-JournalLine $journal "PREREG_SHA256=$ExpectedPreregSha256"
        Write-JournalLine $journal "PARENT_TERMINAL_JOURNAL_SHA256=$ExpectedParentTerminalJournalSha256"
        ...
        Write-JournalLine $journal 'REQUEST_STATE=V3-O3_REQUEST_RESERVED_CONSUMED_NO_RETRY'
        ...
        try {
            ...
            if ($status -ne 200) {
                [System.Collections.Generic.IEnumerable[string]]$locationHeaderValues = $null
                $hasLocation = $response.Headers.TryGetValues('Location', [ref]$locationHeaderValues)
                [string[]]$locationValues = if ($hasLocation) { @($locationHeaderValues) } else { @() }
                $locationEvidence = Get-LocationEvidence -Values $locationValues -RequestUri ([System.Uri]$SourceUrl)
                Write-JournalLine $journal "HTTP_STATUS=$status"
                Write-JournalLine $journal "LOCATION_VALUE_COUNT=$($locationEvidence.ValueCount)"
                Write-JournalLine $journal "LOCATION_PRESENT=$($locationEvidence.Present.ToString().ToLowerInvariant())"
                Write-JournalLine $journal "LOCATION_CLASS=$($locationEvidence.Class)"
                Write-JournalLine $journal "LOCATION_VALUES_SHA256=$($locationEvidence.ValuesSha256)"
                Write-JournalLine $journal "LOCATION_NORMALIZED=$($locationEvidence.Normalized)"
                Write-JournalLine $journal 'RESPONSE_BYTES=0'
                Throw-Guard "HTTP_STATUS_$status"
            }
            ...
            $output = [ordered]@{
                status = 'SUCCESS'
                operation = 'V3-O3'
                source_operations_cumulative = 6
