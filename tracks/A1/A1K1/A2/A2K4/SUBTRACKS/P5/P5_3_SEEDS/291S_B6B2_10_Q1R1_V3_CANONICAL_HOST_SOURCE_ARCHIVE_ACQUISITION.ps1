...
$SourceUrl = 'https://arxiv.org/e-print/2307.12080v2'
...
function Get-LocationEvidence {
    ...
    $host = $resolved.IdnHost.ToLowerInvariant()
    if ($host -ceq 'arxiv.org') { $evidence.Class = 'SAME_ORIGIN_HTTPS' }
    elseif ($host -ceq 'export.arxiv.org') { $evidence.Class = 'ALLOWED_ARXIV_HOST_HTTPS' }
    else {
        $evidence.Class = 'OTHER_OR_INVALID'
        return [pscustomobject]$evidence
    }
...
function Expand-SingleGzipMember {
    ...
    $bounded = [Teoria.Q1R1V3.OneByteReadStream]::new($input)
...
function Invoke-SelfTest {
    ...
    $requestUri = [System.Uri]::new('https://arxiv.org/e-print/2307.12080v2')
...
function Invoke-Acquire {
    ...
    $expectedScript = Join-Path $expectedDirectory '291S_B6B2_10_Q1R1_V3_CANONICAL_HOST_SOURCE_ARCHIVE_ACQUISITION.ps1'
    ...
    $journalPath = Join-Path $expectedDirectory '291J_B6B2_10_Q1R1_V3_SOURCE_ARCHIVE_OPERATION_JOURNAL.txt'
    ...
    foreach ($marker in @(
        'TASK_ID: A2K4-B6B2-10-H-RDIV-C01-RW1-Q1R1-V3-SOURCE-ARCHIVE-ACCESS-AUTHORIZATION-',
        "FROZEN_PREREG291_SHA256: $ExpectedPreregSha256",
        "FROZEN_291S_SHA256: $scriptSha",
        'SOURCE_ACCESS_AUTHORIZED: true_Q1R1_V3_EXACT_ONE_CANONICAL_HOST_ARCHIVE_GET'
    )) {
    ...
    try {
        ...
        Write-JournalLine $journal 'TASK_ID=A2K4-B6B2-10-H-RDIV-C01-RW1-Q1R1-V3-SOURCE-ARCHIVE-ACCESS'
        Write-JournalLine $journal 'OPERATION_ID=V3-O1'
        ...
        Write-JournalLine $journal 'CUMULATIVE_SOURCE_OPERATIONS_BEFORE=3'
        Write-JournalLine $journal 'HISTORICAL_PACKAGES_TOTAL_BEFORE=2'
        Write-JournalLine $journal 'CONSECUTIVE_TECHNICAL_FAILURES_BEFORE=2/10'
        ...
        Write-JournalLine $journal 'REQUEST_STATE=V3-O1_REQUEST_RESERVED_CONSUMED_NO_RETRY'
        ...
        try {
            ...
            $output = [ordered]@{
                status = 'SUCCESS'
                operation = 'V3-O1'
                source_operations_cumulative = 4
