...
> **Workspace boundary (2026-07-16):** `theory/` is the consolidated
> released/release-candidate layer. Existing later `05*` addenda are retained
> in place for historical hashes and links, but are frozen as working-write
> locations. New working rules, questions, status changes, and audit threads
> belong at the applicable depth under `tracks/`; accepted deltas are merged
> into `theory/` only when a release candidate is opened.
