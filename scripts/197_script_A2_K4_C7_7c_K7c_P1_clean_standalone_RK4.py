#!/usr/bin/env python
"""Clean standalone fixed-RK4 regression for A2-K4 K7c P1."""
...
def main() -> int:
    ...
    executed_path_id = "P1_CLEAN_STANDALONE_FIXED_CLASSICAL_RK4_ONLY"
    ...
    def write_checkpoint(grid: int, step: float, endpoint: np.ndarray) -> str:
        ...
        payload = {
            "test": "A2-K4 K7c P1 immutable fixed-RK4 grid checkpoint",
    ...
    regression_checks = {
        ...
        "difference_100_200_matches_frozen_value":
            abs(difference_100_200 - 1.4432726876921487e-6) <= 1e-12,
        "difference_200_400_in_preregistered_quantization_interval":
            3.931235e-6 <= difference_200_400 <= 3.931245e-6,
        "historical_refinement_ratio_reproduced": 0.36 <= refinement_ratio <= 0.375,
        "dominant_component_is_M": dominant_component == "M",
        "nontrivial_finest_change_above_1e-12": normalized_change > 1e-12,
    ...
    regression_checks = {key: bool(value) for key, value in regression_checks.items()}
    ...
    expected_review_reproduced = (
        bool(regression_checks) and all(regression_checks.values())
        and not any(physical_convergence_checks.values())
    )
    output = {
        "test": "A2-K4 K7c P1 clean standalone fixed-RK4 regression",
        "executed_path_id": executed_path_id,
        "derived_from_script_179_sha256":
            "8f45dc698817992e4fb2b859a7cafa49d225b4f7f5fd54b07f88ca99059bd441",
        ...
        "expected_review_reproduced": expected_review_reproduced,
        "execution_verdict": (
            "PASS_K7C_P1_REPRODUCED_EXPECTED_REVIEW"
            if expected_review_reproduced else "REVIEW_K7C_P1_REGRESSION_MISMATCH"
        ),
        "physical_verdict":
            "REVIEW: historical non-convergent M-dominated fixed-RK4 behavior reproduced; no death verdict",
        "fine_depth": "66.5/100",
        "score_effect": "NONE",
        "next_step": "P2 term-by-term M-prime ledger before any new physical claim",
        "scope_limit":
            "0.25 e-fold NID/deep regression only; no four-surface, endpoint-agreement, or full-hierarchy claim",
    ...
    return 0 if expected_review_reproduced else 1
