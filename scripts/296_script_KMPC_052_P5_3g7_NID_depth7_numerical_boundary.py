"""Bounded runner for KMPC-052 NID depth-7 numerical boundary.
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_052_P5_3G7_NID_DEPTH7_NUMERICAL_BOUNDARY.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_052_P5_3G7_NID_DEPTH7_NUMERICAL_BOUNDARY_TECHNICAL_FAILURE.json"
...
EXPECTED_SOURCE_HASHES = {
    ...
    "nid_depth7_numerical_boundary.py": "D5016BD007FD59D4BB715967366774478AFFD344C1B5CD95ACF6F5A9465DD7C8",
}
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-052 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-052 exact source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITE.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-052 prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(smoke: bool, audit: bool, value: float | None) -> None:
    expected = 12.0 if smoke else 45.0 if audit else None
    if expected is None or value is None or not math.isfinite(value) or value != expected:
        raise ValueError(f"KMPC-052 requires exactly {expected} runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC052_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-052 publish fixture")
    ...
    try:
        ...
        if fixture.exists():
            fixture.unlink()
    wrong_smoke_limit = False
    wrong_audit_limit = False
    try:
        _runtime_guard(True, False, 45.0)
    except ValueError:
        wrong_smoke_limit = True
    try:
        _runtime_guard(False, True, 12.0)
    except ValueError:
        wrong_audit_limit = True
    ...
    return {
        ...
        "wrong_smoke_limit_rejected": wrong_smoke_limit,
        "wrong_audit_limit_rejected": wrong_audit_limit,
        "nonfinite_JSON_rejected": nonfinite_rejected,
    }


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="KMPC-052 NID depth-7 same-matrix float64/80-dps numerical boundary audit.")
...
def main() -> int:
    ...
    try:
        _runtime_guard(args.smoke, args.audit, args.max_runtime_seconds)
        ...
        if args.audit:
            ...
            if requested.resolve() != CANONICAL_OUTPUT.resolve():
                raise ValueError("audit requires canonical KMPC-052 --output")
        progress["phase"] = "preimport_guard"
        preimport = _preimport_guard()
        progress["phase"] = "guarded_import"
        from baseScripts.p5_general_synchronous import nid_depth7_numerical_boundary as audit

        if audit.source_hashes() != EXPECTED_SOURCE_HASHES:
            raise RuntimeError("KMPC-052 post-import source hash mismatch")
        if args.smoke:
            progress["phase"] = "runner_smoke"
            runner = _runner_fixtures()
            if not all(runner.values()):
                raise RuntimeError("KMPC-052 runner fixture failed")
            progress["phase"] = "base_smoke"
            payload = audit.run_smoke(args.max_runtime_seconds, RESULT_DIR)
            if not payload["passed"]:
                raise RuntimeError("KMPC-052 exact-matrix smoke failed")
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-052 payload source hash mismatch")
        ...
        print(json.dumps({
            ...
            "V0_pass": safe["V0_reference_parity"]["pass"],
            "V2_pass": safe["V2_single_bounded_refinement"]["pass"],
            "V3_pass": safe["V3_single_80dps_same_matrix_QR"]["pass"],
