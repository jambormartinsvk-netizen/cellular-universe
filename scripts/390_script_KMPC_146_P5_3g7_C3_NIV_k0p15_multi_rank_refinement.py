"""CLI runner for KMPC-146 C3 NIV/.15 multi-rank refinement.
...
from baseScripts.p5_general_synchronous import (
    c3_zero_variant_parallel_v10_niv_k0p15_multi_rank_refinement as c3,
)


RUN_ID = "KMPC-146"
EXPECTED_BASE_SHA256 = (
    "46365EF983E7ECAE53B804E0882730CE96475554533EB0029FAFF12FA5037D91"
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run frozen C3 NIV/.15 four-shard same-matrix multi-rank "
            "refinement."
        )
    )
    ...
    action.add_argument("--worker-smoke-shard", choices=SHARD_CHOICES, help=argparse.SUPPRESS)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.worker_smoke_shard is not None:
        ...
        payload["source_hashes"] = {"base": observed_base_hash, "runner": runner_hash}
    ...
    if args.worker_shard is not None:
        ...
        payload = c3.run_support_worker(
            args.mode, args.k, variant, level, args.max_runtime_seconds, result_dir
        )
    ...
    if args.smoke:
        records = _run_four_workers(
            script_path, args.mode, args.k, result_dir,
            args.max_runtime_seconds, smoke=True,
        )
        payloads = _successful_payloads(records)
        payload = c3.aggregate_smoke_shards(args.mode, args.k, payloads)
    ...
    try:
        records = _run_four_workers(
            script_path, args.mode, args.k, result_dir,
            args.max_runtime_seconds, smoke=False,
        )
        if not _records_pass(records):
            raise RuntimeError("one or more KMPC-146 support workers failed")
        ...
        failure_payload = {
            ...
            "identity": {
                ...
                "physical_receipt": (
                    "four_support_shards_gamma0_af0_rank104_130_"
                    "refinement_pair"
                ),
            },
            ...
            "source_hashes": {"base": observed_base_hash, "runner": runner_hash},
            "score_effect": "NONE",
    ...
    print(json.dumps({
        "run_id": RUN_ID,
        "output": str(output),
        "candidate_interpretation_not_verdict": payload["candidate_interpretation_not_verdict"],
