"""CLI runner for KMPC-137 BI/.15 fuel-ordered two-wave exact resume.
    ...
    for variant, level in c3.SHARDS:
        ...
        if not smoke and level == "audit":
            if handoff_dir is None:
                raise RuntimeError("KMPC-137 handoff directory missing")
            command.extend([
                "--handoff-output",
    ...
    for variant in VARIANT_CHOICES:
        ...
        if not smoke:
            if coefficient_payloads is None or handoff_dir is None:
                raise RuntimeError("KMPC-137 exact command handoff inputs missing")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.worker_coefficient_shard is not None:
        ...
        if level == "audit":
            if args.handoff_output is None or not isinstance(handoff, dict):
                raise RuntimeError("KMPC-137 audit worker handoff output missing")
            ...
            raise RuntimeError("KMPC-137 accepted worker emitted audit handoff")
        else:
    ...
    if args.worker_exact_variant is not None:
        if (
            ...
            raise RuntimeError("KMPC-137 exact worker handoff arguments missing")
    ...
    try:
        with tempfile.TemporaryDirectory(
            prefix="KMPC137_HANDOFF_", dir=result_dir
        ) as temporary_name:
            ...
            if not _records_pass(coefficient_records, 4):
                raise RuntimeError("KMPC-137 coefficient wave incomplete")
            ...
            if remaining <= 0.0:
                raise TimeoutError("KMPC-137 no parent budget for exact wave")
            exact_records = _run_commands(
                _exact_commands(
                    script_path,
                    args.mode,
            ...
            if not _records_pass(exact_records, 2):
                raise RuntimeError("KMPC-137 exact-boundary wave incomplete")
        ...
        if not temporary_removed:
            raise RuntimeError("KMPC-137 temporary handoff directory not removed")
        ...
        if time.monotonic() - parent_started > PARENT_TOTAL_WALL_LIMIT_SECONDS:
            raise TimeoutError("KMPC-137 parent total wall limit exceeded")
        ...
        failure_payload = {
            ...
            "identity": {
                ...
                "physical_receipt": (
                    "BI_K0p15_fuel_ordered_two_wave_HP_M1_exact_resume"
                ),
            },
