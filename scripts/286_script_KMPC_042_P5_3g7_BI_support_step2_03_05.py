"""Bounded runner for KMPC-042 GLOBAL_C1 / BI_SUPPORT_STEP_2."""
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_042_P5_3G7_BI_SUPPORT_STEP_2_03_05.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_042_P5_3G7_BI_SUPPORT_STEP_2_03_05_TECHNICAL_FAILURE.json"
...
EXPECTED_PREREQUISITES = {
    "RUN_KMPC_041_P5_3G7_BI_C1_PRIMARY_EXTENDED_COVERAGE.json": "8BB006EF6606476D85168FBDCD913249E9EDE024C1017473376A33CF4C7AE183",
}
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-042 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-042 exact pre-import source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-042 immutable prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-042 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC042_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-042 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded KMPC-042 BI support step 2: regression [0,1]/[0,3], "
            "candidate [0,3], audit [0,5], common 0..3, tail 4,5."
        )
    )
...
def main() -> int:
    ...
    try:
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-042 payload source hash mismatch")
            ...
            json.dumps(
                {
                    ...
                    "regression_pass": safe["regression_against_KMPC041"]["pass"],
