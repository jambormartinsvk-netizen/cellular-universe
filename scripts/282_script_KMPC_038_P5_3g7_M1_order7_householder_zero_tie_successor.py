"""Bounded runner for KMPC-038 Householder zero-tie technical successor."""
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_038_P5_3G7_M1_ORDER7_HOUSEHOLDER_ZERO_TIE.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_038_P5_3G7_M1_ORDER7_HOUSEHOLDER_ZERO_TIE_TECHNICAL_FAILURE.json"
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-038 output conflict: {conflicts}")
    actual = {name: _sha256_file(BASE_DIR / name) for name in EXPECTED_SOURCE_HASHES}
    if actual != EXPECTED_SOURCE_HASHES:
        raise RuntimeError("KMPC-038 exact pre-import source hash mismatch")
    if _sha256_file(PREREQUISITE) != EXPECTED_PREREQUISITE_HASH:
        raise RuntimeError("KMPC-038 immutable prerequisite hash mismatch")
    if _sha256_file(PF072_FAILURE) != EXPECTED_PF072_HASH:
        raise RuntimeError("KMPC-038 PF-072 lineage hash mismatch")
    if _sha256_file(MPMATH_LINALG) != EXPECTED_MPMATH_LINALG_HASH:
        raise RuntimeError("KMPC-038 mpmath linalg source hash mismatch")
    return actual
...
def _runtime_guard(value: float | None, expected: float) -> bool:
    if value is None or not math.isfinite(value) or value != expected:
        raise ValueError(f"KMPC-038 requires exactly {expected} runtime seconds")
    return True


def _runner_smoke_fixtures() -> dict[str, bool]:
    fixture_target = RESULT_DIR / ".KMPC038_publish_collision_fixture.json"
    fixture_temp = fixture_target.parent / f".tmp-{fixture_target.name}"
    if fixture_target.exists() or fixture_temp.exists():
        raise FileExistsError("stale KMPC-038 publish fixture")
    try:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded KMPC-038 Householder sign(0) technical successor only; "
    ...
    try:
        ...
        if args.audit:
            ...
            if args.output.resolve() != CANONICAL_OUTPUT.resolve():
                raise ValueError("noncanonical KMPC-038 output path")
        ...
        from baseScripts.p5_general_synchronous import (
            m1_order7_numerical_refinement_v2_householder as audit,
        )

        if audit.source_hashes() != EXPECTED_SOURCE_HASHES:
            raise RuntimeError("KMPC-038 post-import source hash mismatch")
        if args.smoke:
            progress["phase"] = "runner_smoke"
            runner_fixtures = _runner_smoke_fixtures()
            if not all(runner_fixtures.values()):
                raise RuntimeError("KMPC-038 runner fixture failed")
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-038 payload source hash mismatch")
        ...
        failure = {
            ...
            "pf072_lineage_expected": EXPECTED_PF072_HASH,
            "mpmath_linalg_expected": EXPECTED_MPMATH_LINALG_HASH,
            "score_effect": "NONE",
