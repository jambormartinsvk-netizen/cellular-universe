#!/usr/bin/env python
"""Exact local CAMB symbolic hierarchy coefficient cross-check for K4.3b."""
...
def j_expected(ell: int):
    current = cs.pi_g if ell == 2 else cs._make_index_func("J", ell)
    previous = cs.q_g if ell - 1 == 1 else cs._make_index_func("J", ell - 1)
    following = cs._make_index_func("J", ell + 1)
    rhs = -cs.k / (2 * ell + 1) * (
...
def g_expected(ell: int):
    current = cs.pi_r if ell == 2 else cs._make_index_func("G", ell)
    previous = cs.q_r if ell - 1 == 1 else cs._make_index_func("G", ell - 1)
    following = cs._make_index_func("G", ell + 1)
    rhs = -cs.k / (2 * ell + 1) * (
        (ell + 1) * cs.Kf[ell] * following - ell * previous
    )
    if ell == 2:
...
def main() -> int:
    ...
    for ell in range(2, 9):
        if time.monotonic() - started > args.max_runtime_seconds:
            raise TimeoutError("internal exact CAMB hierarchy deadline exceeded")
        comparisons = {
    ...
    polter_residual = sp.simplify(cs.polter_sub.rhs - expected_polter)
    residuals["polarization_source"] = str(polter_residual)
    checks["polarization_source"] = bool(polter_residual == 0)

    passed = all(checks.values())
    output = {
        "test": "A2-K4.3b exact local CAMB symbolic hierarchy coefficient cross-check",
        "CAMB_version": camb.__version__,
        "ell_range": [2, 8],
        "exact_checks": len(checks),
        "checks": checks,
        "symbolic_residuals": residuals,
        "verdict": "PASS_EXACT_CAMB_HIERARCHY_COEFFICIENTS" if passed else "COEFFICIENT_MISMATCH_REVIEW",
        "scope": "J temperature, G massless-neutrino, E-polarization hierarchies and polter source; no K4 evolution",
