"""Bounded runner for preregistered KMPC-032 S-C0 coefficient passport."""
...
def _validate_hashes() -> dict[str, str]:
    ...
    if observed != EXPECTED_BASE_HASHES:
        raise RuntimeError(f"frozen KMPC-032 base hash mismatch: {observed}")
    return observed
...
def main() -> int:
    ...
    try:
        ...
        from scripts.baseScripts.p5_general_synchronous import s_c0_coefficient_passport as audit
        ...
        failure = {
            "test": "KMPC-032 S-C0 coefficient passport technical failure",
            "run_id": "KMPC-032",
