"""Bounded KMPC-046 owner/stderr-only successor for KMPC-045 / PF-074."""
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_046_P5_3G7_BI_SUPPORT_STEP_3_OWNER_SUCCESSOR.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_046_P5_3G7_BI_SUPPORT_STEP_3_OWNER_SUCCESSOR_TECHNICAL_FAILURE.json"
...
EXPECTED_SOURCE_HASHES = {
    ...
    "bi_c1_coverage.py": "303515C80945905BFC537B8FFDB94F1F126B73EB939679D46A95DD4BDE384BF6",
    "cdi_support_ladder.py": "A8257E2195E2AB61B5C4195B80EA44EE7669B5A52F9C8440BA5F5244190E3068",
    "bi_support_step2.py": "08A8071D5D3DC8A1A0D58CB76CAF08548ADC6D85F067D51151CE78129CC1F19F",
    ...
    "bi_support_step3.py": "1ABB16A886432C4A2B908CE802598D4970567030C2E7CCAFE6FA1A37A4C36CC8",
    "bi_support_step3_v2_owner.py": "EB434319DA1E07AAE23B2CE76F6287934B941FF5A7835AF9CDE702AECA6E5EDB",
}
EXPECTED_PREREQUISITES = {
    "RUN_KMPC_042_P5_3G7_BI_SUPPORT_STEP_2_03_05.json": "E5F18DA4DE5A718C4448D095804F6D41FE88445A95FB99645EFBCCB48D48CA61",
    "RUN_KMPC_043_P5_3G7_BI_M1_ORDER7_PROVENANCE_GATE.json": "B02D1D16CFAE4331378B68F12258142F84A424419BB9D3A52AAEE87D0CC61EB0",
    "RUN_KMPC_044_P5_3G7_BI_M1_ORDER7_NUMERICAL_BOUNDARY.json": "C3BD732C9F3FB402E4143DA6EF149E6C2830F5F5C96D17D21D314BC5B82F1C36",
    "RUN_KMPC_045_P5_3G7_BI_SUPPORT_STEP_3_05_07_TECHNICAL_FAILURE.json": "FFFF061651A06F3FD097F5C6622C42084643F41D98C2C3B2B0C141A54C330C01",
}
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-046 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-046 exact pre-import source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-046 immutable prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-046 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC046_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-046 publish fixture")
    ...
    capture = StringIO()
    synthetic_failure = {
        "run_id": "KMPC-046-SMOKE",
        "execution_status": "TECHNICAL_FAILURE_NO_PHYSICS_VERDICT",
    }
    with redirect_stderr(capture):
        print(
            json.dumps(_json_safe(synthetic_failure), sort_keys=True),
            file=sys.stderr,
            flush=True,
        )
    stderr_failure_route = json.loads(capture.getvalue()) == synthetic_failure
    return {
        ...
        "stderr_failure_route_serializes": stderr_failure_route,
    }
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "KMPC-046 PF-074 owner/stderr-only successor: unchanged BI support "
            "step 3 [0,5] to [0,7], no [0,9]."
        )
    )
    ...
    try:
        ...
        if args.audit:
            ...
            if requested.resolve() != CANONICAL_OUTPUT.resolve():
                raise ValueError("audit requires canonical KMPC-046 --output")
        progress["phase"] = "preimport_guard"
        preimport = _preimport_guard()
        progress["phase"] = "guarded_import"
        from baseScripts.p5_general_synchronous import bi_support_step3_v2_owner as audit

        if audit.source_hashes() != EXPECTED_SOURCE_HASHES:
            raise RuntimeError("KMPC-046 post-import source hash mismatch")
        if args.smoke:
            progress["phase"] = "runner_smoke"
            runner = _runner_fixtures()
            if not all(runner.values()):
                raise RuntimeError("KMPC-046 runner fixture failed")
            progress["phase"] = "base_owner_smoke"
            payload = audit.run_smoke(args.max_runtime_seconds, RESULT_DIR)
            if not payload["passed"]:
                raise RuntimeError("KMPC-046 base smoke failed")
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-046 payload source hash mismatch")
            ...
            json.dumps(
                {
                    ...
                    "regression_pass": safe["regression_against_KMPC042"]["pass"],
        ...
        print(
            json.dumps(_json_safe(failure), sort_keys=True),
            file=sys.stderr,
            flush=True,
        )
        return 2
