"""Bounded runner for preregistered KMPC-031 no-solve tail provenance."""
...
WRAPPER_PATH = BASE_DIR / "full_ra_m3_seed_attempt10_tail_provenance.py"
EXPECTED_WRAPPER_HASH = "A7C06D4C16AF5429319DFF307ADB4A2FCF72542AA65E92B1D6EA1B229387CA55"
...
def _write_new(path: Path, payload: dict[str, object]) -> None:
    ...
    if path.parent != RESULT_DIR.resolve():
        raise ValueError("canonical output must be directly inside result directory")
    if path.exists():
...
def main() -> int:
    ...
    try:
        ...
        from scripts.baseScripts.p5_general_synchronous import full_ra_m3_seed as physics
        from scripts.baseScripts.p5_general_synchronous import (
            full_ra_m3_seed_attempt10_tail_provenance as audit,
        )
        ...
        if observed_wrapper_hash != EXPECTED_WRAPPER_HASH:
            raise RuntimeError(
                "attempt-10 wrapper hash mismatch: "
        ...
        failure_path = RESULT_DIR / (
            "RUN_KMPC_031_P5_3G7_M3_FULL_RA_DEEP_TAIL_BRANCH_PROVENANCE_"
            "TECHNICAL_FAILURE.json"
        )
        failure = {
            "test": "KMPC-031 attempt-10 technical failure evidence",
            "run_id": "KMPC-031",
