"""Bounded runner for preregistered KMPC-034 CDI C1 coverage."""
...
def _validate_hashes() -> dict[str, str]:
    ...
    if observed != EXPECTED_BASE_HASHES:
        raise RuntimeError(f"frozen KMPC-034 base hash mismatch: {observed}")
    return observed
...
def main() -> int:
    ...
    try:
        ...
        from scripts.baseScripts.p5_general_synchronous import cdi_c1_coverage as audit
        ...
        print(json.dumps({
            ...
            "core_pass": payload["core_pass"],
            "common_coefficient_bridge_pass": payload["common_coefficient_bridge_pass"],
            "pure_tail_pass": payload["pure_tail_pass"],
        }, sort_keys=True))
        return 0
    except Exception as error:
        failure_path = RESULT_DIR / (
            "RUN_KMPC_034_P5_3G7_CDI_C1_PRIMARY_EXTENDED_COVERAGE_TECHNICAL_FAILURE.json"
        )
        failure = {
            "test": "KMPC-034 CDI C1 technical failure",
            "run_id": "KMPC-034",
