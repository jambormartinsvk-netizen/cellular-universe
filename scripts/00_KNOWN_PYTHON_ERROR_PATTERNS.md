...
## 18. PowerShell lokálna premenná koliduje s automatickou premennou

**Príklad:** PF-132, `$host` verzus read-only automatická `$Host`.

**Symptóm:** syntax a statický diff prejdú, ale priradenie skončí
`Cannot overwrite variable ... because it is read-only or constant`.
PowerShell názvy premenných, aliasov a kľúčových slov treba posudzovať bez
ohľadu na veľkosť písmen.

**Kontrola:** pred freeze zozbierať všetky mená, do ktorých skript zapisuje,
a case-insensitive ich porovnať s automatic/preference variables, aliasmi a
keywords. Behaviorálny SelfTest musí dosiahnuť každú vetvu, ktorá zapisuje
novú lokálnu premennú; pri URI klasifikácii aspoň same-origin, povolený iný
host a zamietnutý host.

**Rozsudok:** frozen skript `DO_NOT_RUN_TECHNICAL`; neupravovať ani
nespúšťať znova. Versioned successor smie urobiť iba auditovaný rename a
musí zachovať pôvodné zlyhané fixtures ako regresiu.
