"""KMPC-134 BI/.15 C3 HP-M1 checkpoint exact-resume adapter.

Theory author: Martin Jambor. Script creator: Codex (OpenAI).
The adapter restores frozen HP-M1 state, recomputes variant F0, and permits
80-dps evidence to supersede only the audit M3 driver and non-fit holdout.
"""
...
def output_name(mode: str, k_mpc: float) -> str:
    if (mode, k_mpc) != TARGET:
        raise ValueError("KMPC-134 is frozen to BI/k=0.15")
    return "RUN_KMPC_134_P5_3G7_C3_BI_K0p15_ZERO_VARIANT_PAIR_HP_M1_EXACT_RESUME.json"
...
def _source_hashes() -> dict[str, str]:
    ...
    hashes.update({
        ...
        "c3_zero_variant_parallel_v6_bi_k0p15_hp_m1_exact_resume.py": sha256_file(
...
def _exact_resume_solve(
    ...
    exact_evidence: dict[str, object] = {
        "applicable": False,
        "superseded_checks": [],
        "checks": {"accepted_rank_not_exact_resumed": level == "accepted"},
        "pass": level == "accepted",
    }
    if level == "accepted":
        solve = scientific.c2._solve_support(
            mode, k_mpc, support, inputs, hp_m1, deadline
        )
    ...
    if not v11._owners_restored():
        raise RuntimeError("KMPC-134 float capture owners were not restored")
...
def run_support_worker(
    ...
    if (mode, k_mpc) != TARGET or (variant, level) not in SHARDS:
        raise ValueError("KMPC-134 worker identity outside preregistration")
    ...
    solve, exact_evidence = _exact_resume_solve(
        mode, k_mpc, variant, level, hp_m1, deadline
    )
    deadline()
    shared_checks = {
    ...
    payload = {
        "run_id": RUN_ID,
        "worker_role": "C3_BI_K0P15_HP_M1_EXACT_RESUME_SHARD",
        "identity": {
            "mode": mode,
            "k_Mpc_inverse": k_mpc,
            "variant": variant,
    ...
    if not scientific.c2.finite_owner._all_finite(payload):
        raise FloatingPointError("non-finite KMPC-134 worker payload")
    return payload
...
def run_worker_smoke(
    ...
    if (mode, k_mpc) != TARGET or (variant, level) not in SHARDS:
        raise ValueError("invalid KMPC-134 smoke shard")
    nominal = scientific._load_nominal_reference(result_dir, mode, k_mpc)
    ...
    return {
        "run_id": RUN_ID,
        "worker_role": "C3_BI_K0P15_HP_M1_EXACT_RESUME_SHARD_SMOKE",
        "identity": {
            "mode": mode,
            "k_Mpc_inverse": k_mpc,
            "variant": variant,
...
def _require_shard(
    ...
    if payload.get("run_id") != RUN_ID:
        raise RuntimeError(f"KMPC-134 run identity mismatch: {variant}/{level}")
    if payload.get("worker_role") != "C3_BI_K0P15_HP_M1_EXACT_RESUME_SHARD":
        raise RuntimeError(f"KMPC-134 worker role mismatch: {variant}/{level}")
    if payload.get("identity") != {
        ...
        raise RuntimeError(f"KMPC-134 shard identity mismatch: {variant}/{level}")
    evidence = payload.get("exact_resume_evidence")
    if level == "audit" and (
        not isinstance(evidence, dict) or evidence.get("pass") is not True
    ):
        raise RuntimeError(f"KMPC-134 exact audit evidence failed: {variant}")
...
def aggregate_shards(
    ...
    if set(shards) != expected:
        raise RuntimeError("KMPC-134 exact four-shard register mismatch")
    ...
    payload["test"] = "A2-K4 P5.3g7 C3 BI/.15 HP-M1 exact-resume pair"
    payload["identity"]["physical_receipt"] = (
        "four_support_shards_gamma0_af0_HP_M1_exact_resume_pair"
    )
...
def aggregate_smoke_shards(
    ...
    for variant, level in SHARDS:
        ...
        checks[key] = bool(
            ...
            == "C3_BI_K0P15_HP_M1_EXACT_RESUME_SHARD_SMOKE"
