"""Bounded BI GLOBAL_C1 primary/extended atom for KMPC-041.

The module introduces no equation, parameter, support, surface, or threshold.
It applies the frozen R-A solver to BI only and evaluates F0 and M3 common
coefficients plus cancellation-safe added-power envelopes.
"""
...
RUN_ID = "KMPC-041"
OUTPUT_NAME = "RUN_KMPC_041_P5_3G7_BI_C1_PRIMARY_EXTENDED_COVERAGE.json"
FAILURE_NAME = "RUN_KMPC_041_P5_3G7_BI_C1_PRIMARY_EXTENDED_COVERAGE_TECHNICAL_FAILURE.json"
MODE = "BI"
K_MPC = 0.05
VARIANT = "nominal"
PRIMARY = (0, 1)
...
def source_hashes() -> dict[str, str]:
    ...
    hashes = dict(c1.source_hashes())
    hashes["bi_c1_coverage.py"] = sha256_file(here / "bi_c1_coverage.py")
    return hashes


def make_deadline(limit: float) -> tuple[float, Callable[[], None]]:
    if not math.isfinite(limit) or limit <= 0.0 or limit > 4.8:
        raise ValueError("KMPC-041 runtime must be in (0, 4.8] seconds")
    started = time.monotonic()

    def deadline() -> None:
        if time.monotonic() - started > limit:
            raise TimeoutError("KMPC-041 BI C1 internal deadline exceeded")
...
def _support_guard() -> dict[str, object]:
    ...
    checks = {
        "contract_primary_exact_01": tuple(spec["primary"]) == PRIMARY,
        "implementation_primary_equals_contract": implementation_primary
        == tuple(spec["primary"]),
        "derived_extended_exact_03": implementation_extended == EXTENDED,
        "derived_extended_equals_contract": implementation_extended
        == tuple(spec["extended"]),
        "leading_j_exact_1": derived["leading_j"] == spec["leading_j"] == LEADING_J,
        "F0_counts_exact_4_8": bool(
            derived["F0_primary"]
            == spec["f0_primary"]
            == physics.EXPECTED_F0_PRIMARY[MODE]
            == 4
            and derived["F0_extended"]
            == spec["f0_extended"]
            == physics.EXPECTED_F0_EXTENDED[MODE]
            == 8
        ),
        "M3_counts_exact_26_52": bool(
            derived["M3_primary"]
            == spec["m3_primary"]
            == physics.EXPECTED_M3_PRIMARY[MODE]
            == 26
            and derived["M3_extended"]
            == spec["m3_extended"]
            == physics.EXPECTED_M3_EXTENDED[MODE]
            == 52
        ),
...
def _pure_added_tail(
    ...
    for z in physics.Z_SURFACES:
        ...
        for name in state_order:
            ...
            states[name] = {
                "base_primary_01": base,
                "signed_added_tail_23_diagnostic": signed_added,
                "absolute_term_envelope_23_authoritative": envelope,
                "full_extended_03": full,
    ...
    return {
        ...
        "expected_added_powers": [2, 3],
        "authoritative_metric": "sum(abs(c_j)*z**j) for j=2,3",
        "signed_tail_role": "DIAGNOSTIC_ONLY",
        "by_z": by_z,
        "all_finite": all_finite,
        "pass": bool(all_pass and all_finite and added_powers == (2, 3)),
    }
...
def run_smoke(max_runtime_seconds: float, result_dir: Path) -> dict[str, object]:
    ...
    wrong_rejected = not collective_contract.validate_candidate(wrong).valid
    ...
    checks = {
        ...
        "wrong_BI_extended_support_rejected": wrong_rejected,
        "fixed_identity": MODE == "BI" and K_MPC == 0.05 and VARIANT == "nominal",
...
def run_audit(max_runtime_seconds: float, result_dir: Path) -> dict[str, object]:
    ...
    core = c1._diagnostic_core(result)
    s_c0_guard = c1._s_c0_actual_coefficient_guard(result)
    common = {
        "F0": _common_bridge(
            result["fuel_primary"]["state"], result["fuel_extended"]["state"]
        ),
        "M3": _common_bridge(
            result["m3_primary"]["fractional_state"],
            result["m3_extended"]["fractional_state"],
        ),
    }
    common_pass = all(row["pass"] for row in common.values())
    tails = {
        "F0": _pure_added_tail(
            result["fuel_extended"]["state"], tuple(sorted(result["fuel_extended"]["state"]))
        ),
        "M3": _pure_added_tail(
            result["m3_extended"]["fractional_state"],
    ...
    core_checks = {
        ...
        "BI_core_gates": bool(core["pass"]),
        "S_C0_actual_primary_extended_lower_moments": bool(s_c0_guard["pass"]),
        "all_common_tail_and_solve_fields_finite": _all_finite(
            {"result": result, "common": common, "tails": tails}
        ),
    }
    core_pass = all(core_checks.values())
    if not core_pass:
        candidate = "REVIEW_BI_C1_CORE_GATE_UNCLOSED"
    elif not common_pass:
        candidate = "REVIEW_BI_C1_COMMON_COEFFICIENT_CONVERGENCE_UNCLOSED"
    elif not tail_pass:
        candidate = "REVIEW_BI_C1_SUPPORT_EXTENSION_REQUIRED"
    else:
        candidate = "PASS_BI_C1_PRIMARY_EXTENDED_ATOM_CANDIDATE_ONLY"
    deadline()
    payload = {
        "test": "A2-K4 P5.3g7 GLOBAL_C1 BI primary [0,1] versus extended [0,3]",
        "run_id": RUN_ID,
        ...
        "scope": {
            "included": "BI primary [0,1] versus extended [0,3], F0/M3 common 0,1 and tail 2,3",
            "excluded": "CDI transfer, NID/NIV, other k/variants, S-M, full hierarchy, finite opacity, ODE, P5.4, G8/G9, CLASS/CMB/BBN/S8/H0",
        },
        "support_count_guard": support_guard,
        "immutable_prerequisites": {
            "S_C0": {"file": S_C0_RESULT[0], "sha256": passport_hash},
            "previous_coverage": {
                "file": PREVIOUS_COVERAGE_RESULT[0], "sha256": previous_hash,
        ...
        "common_coefficient_bridges_01_03": common,
        "common_coefficient_pass": common_pass,
        "pure_added_tails_23": tails,
    ...
    if not _all_finite(payload):
        raise FloatingPointError("non-finite value in final KMPC-041 payload")
    return payload
