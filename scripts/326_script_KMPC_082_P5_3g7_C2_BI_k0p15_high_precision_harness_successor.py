"""Runtime-harness successor for KMPC-082 C2 BI/k=.15 80-dps boundary.
...
if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-082 deterministic single-thread environment not active")

audit.configure(
    run_id="KMPC-082", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_082_P5_3G7_C2_BI_K0p15_HIGH_PRECISION_HOLDOUT_BOUNDARY.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        ...
        aggregate_name="NOT_AVAILABLE_KMPC_082.json",
