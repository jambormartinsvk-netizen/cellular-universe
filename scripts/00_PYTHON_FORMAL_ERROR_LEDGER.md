...
| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-111 | 354 / KMPC-110 smoke | compile/help prešli; smoke skončil v guarded-import resume fixture na `KMPC-106 serialized M1 order mismatch`; official ani exact resume sa nespustili | checkpoint raw bol korektne publikovaný cez `sort_keys=True`, preto poradie vnoreného dictu `hp_m1_state_decimal` po načítaní nie je autoritatívne; V17 mal použiť osobitné `m1_state_order/fuel_state_order`, ale odovzdal raw dict priamo order-sensitive helperu | nie; iba SHA načítanie a pokus o deserializáciu pred audit solve, matrix capture a exact driver | runner 354 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-111 smie pridať iba V18 order-reconstruction overlay podľa checkpointových order listov a delegovať byteovo nezmenený V17 | každý JSON checkpoint s `sort_keys=True` musí obnovovať register z explicitného hashovaného order poľa; poradie objektových kľúčov po parse nie je proveniencia registra |

## Dodatok PF-112

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-112 | 355 / KMPC-111 official | compile/help a 19 smoke checks prešli; official po audit solve a exact-boundary výpočte skončil fail-closed iba na `audit_payload_field_parity`; failure SHA `1ADCB30A3A41AC38EF29E454016AE164A05CC8DD489D6F254B26A91C3BD95E40` | V17 porovnal živý Python audit payload s JSON-načítaným KMPC-108 payloadom pred aplikovaním stabilnej publish kanonizácie; serializer mení integer dict kľúče na stringy a tuple na listy, hoci vedecké hodnoty nemení | drahý audit aj exact boundary prebehli iba v pamäti, ale technická parity brána zabránila payloadu; výsledky sa nesmú interpretovať ani použiť ako verdikt | runner 355 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-112 smie iba pred parity porovnaním aplikovať presne tú istú JSON-safe reprezentáciu (`dict key -> str`, `tuple -> list`, mpf -> decimal90) ako immutable publish harness; V17/V18, matematika, checkpoint, prahy a runtime ostávajú byteovo nezmenené | field parity medzi živým objektom a publikovaným JSON sa musí porovnávať v jednej predregistrovanej kanonickej reprezentácii; typové zmeny serializeru nie sú vedecké rozdiely, ale musia byť explicitne fixture-testované |

## Dodatok PF-113

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-113 | 358 / KMPC-114 prvé smoke CLI volanie | compile a `--help` prešli; volanie s neexistujúcim starším prepínačom `--smoke-test` skončilo parser exit `2` a zároveň chýbal povinný `--max-runtime-seconds`; official sa nespustil a raw nevznikol | operátorský príkaz nebol po zobrazení aktuálneho CLI preložený na kontrakt harnessu `--smoke --max-runtime-seconds 4.8` | nie; argparse zastal pred `run_smoke`, checkpointom, M1 solve a fyzikou | nejde o chybu immutable runnera ani rovníc; ten istý runner 358 sa smie použiť so správnym CLI; do vecného úspechu je aktívny technický counter `1/10` | po `--help` zostaviť smoke/official príkaz výlučne z aktuálne zobrazených prepínačov a pred spustením mechanicky overiť povinný runtime argument |

## Dodatok PF-114

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-114 | 368 / KMPC-124 smoke | compile/help prešli; smoke skončil exit `2` na `checkpoint_complete=false`, success raw KMPC-124 nevznikol | KMPC-123 vytvoril verdict-free checkpoint attempt, ale nový accepted `[-1,6]` solve mal M3 driver `fuel_Euler[6]=1.4819148859e-10 >1e-10`; preto 8/9 preconditions true a checkpoint nebol platný na resume | nie v KMPC-124; guard načítal iba immutable KMPC-123 metadata a zastal pred restore/audit solve | runner 368 je `DO_NOT_RUN_AUDIT_TECHNICAL`; raw KMPC-123 ostáva immutable incomplete checkpoint bez verdiktu; nástupca musí v novom behu priamo spojiť wider support so same-matrix refinement a nesmie obísť checkpoint guard | pred predregistráciou resume mechanicky kontrolovať nielen status/candidate/hash, ale aj `checkpoint_complete=true` a všetky `checkpoint_preconditions`; checkpoint official exit `0` sám nestačí na jeho použiteľnosť |

## Dodatok PF-115

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-115 | 369 / KMPC-125 official | compile/help/smoke prešli; official po accepted/audit výpočtoch skončil exit `2`, `KeyError('same_matrix_refinement')`; failure raw SHA `1ED339AE...A31D0`, success raw nevznikol | immutable refinement base aplikoval corrections iba pri `expected_rank=104`; widened accepted M3 má rank 104, ale audit M3 rank 130 preto nemal očakávané provenance pole, ktoré post-processing bez guardu čítal | áno iba v pamäti, ale payload neprešiel post-processingom a nesmie sa interpretovať ako fyzikálny výsledok | runner 369 je `DO_NOT_RUN_AUDIT_TECHNICAL`; versioned successor smie explicitne povoliť iba ranky `{104,130}`, refinovať a auditovať oba a publikovať oddelenú accepted/audit provenance; matematika a prahy ostávajú nezmenené | pred official behom musí smoke fixture pokryť všetky predregistrované target ranky a post-processing musí kontrolovať prítomnosť provenance pre každý očakávaný solve, nie iba historický rank 104 |

## Dodatok PF-116

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-116 | prvý EA-029 behavior-preflight orchestration príkaz | PowerShell `ParserError: Missing statement block after 'clean'`; parser zastal pred vytvorením fresh-copy vetvy a pred Python procesom | pomocná funkcia bola pomenovaná `Clean`, čo v novšom PowerShelli case-insensitive koliduje s jazykovým blokom `clean` | nie; package, rawy aj verdikty ostali nedotknuté | opravený preflight použije nekolidujúci názov `Normalize-JsonValue`; nejde o chybu runnera 371 a route-local technical counter sa nemení | orchestration helpery pomenovať schváleným Verb-Noun tvarom a nepoužívať názvy zhodné s PowerShell language keywords |

## Dodatok PF-117

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-117 | 372 / KMPC-128 prvý AD/.005 C3 pair official | compile base/runner, help a tri AD schema smoke prešli; official dokončil `gamma0`, no počas `af0` prekročil spoločný interný limit `4.8 s`; immutable failure SHA `E974DEE195641A68CA753074E73D416E53EFC471A09473929560950E8825E3D9` | dve úplné nulové variantové solve boli síce logicky oddelené, ale zdieľali jeden procesný deadline; spojenie znížilo počet súborov, nie kumulovaný runtime | iba čiastkový `gamma0` výsledok vznikol v pamäti; párový success payload nevznikol a failure receipt explicitne nesie `NONE_TECHNICAL_FAILURE` | runner 372 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-129 smie zmeniť iba procesnú architektúru na dva izolované paralelné variantové workery, každý s vlastným `4.8 s` limitom, a jeden finálny pair receipt; rovnice, support, vstupy, prahy a 45-logický register ostávajú nezmenené | ak jeden fyzický receipt obsahuje viac plných solve vetiev, smoke musí modelovať kumulovaný runtime alebo ich spustiť v samostatných ohraničených worker procesoch; znižovanie počtu výstupov nesmie odstrániť per-solve deadline |

## Dodatok PF-118

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-118 | 373 / KMPC-129 prvý paralelný smoke | compile base/runner a help prešli; oba smoke workery vrátili exit `2`, parent register bol `0/2`, bez raw súboru | `run_worker_smoke` vložil diagnostický fakt `physics_executed=False` priamo do mapy checks a následne použil `all(checks.values())`; korektná kontrola mala byť `no_physics_executed=True` | nie; workery iba načítali a hashovo overili nominal schému, žiadny M1/F0/M3 solve nebežal | runner 373 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-130 smie pridať iba identity/smoke wrapper, ktorý opraví bool význam a zachová byteovo rovnakú KMPC-129 paralelnú worker/aggregate implementáciu | boolean check mapa musí obsahovať splnené výroky, nie raw stavové hodnoty; pri negatívnom očakávaní používať explicitný názov `no_*` a hodnotu `observed is False` |

## Dodatok PF-119

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-119 | 374 / KMPC-130 AD/.005 official | compile/help/smoke prešli; paralelný `gamma0` worker technicky dobehol za `4.7960 s` a jeho in-memory payload mal všetky brány true, ale `af0` worker prekročil vlastný `4.8 s` deadline; failure SHA `01E9498EC21C9BA2229CE77416A0E906FF804BA7CF4D7C898CC7CC252EFFB5C6` | izolácia po variantoch odstránila spoločný deadline, ale každý worker stále kumuloval nominal hash load, B1/TCA0, M1, accepted aj audit solve a post-checks; `af0` má navyše nulový a coefficient bridge | iba `gamma0` partial payload je zachovaný vo failure receipt; finálny pair success raw nevznikol a fyzikálny verdikt je `NONE_TECHNICAL_FAILURE` | runner 374 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-131 smie zmeniť iba procesnú granularitu na štyri paralelné `variant × accepted/audit` support workery a parent-only common/tail/null/bridge merge; jeden finálny receipt, rovnice a prahy nezmenené | runtime smoke musí mať bezpečnú rezervu, nie iba tesný PASS; ak jeden worker dosiahne >90 % limitu alebo druhý timeoutne, rozdeliť drahé support solve a post-processing do menších izolovaných fáz bez zvýšenia limitu |

## Dodatok PF-120

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-120 | 378 / KMPC-134 BI/.15 official | compile/help/smoke prešli; všetky štyri official workery prekročili vlastný interný limit `4.8 s`; immutable failure SHA `F4BBC3CE6FB0C284E355075189ED79E8B9F047BCCFFF5ACEC178D7927B0310A2` | obnovený decimal80 `mpf` HP-M1 register bol odovzdaný aj do obyčajného variantového coefficient solve, čím sa numericky nezmenená prípravná fáza presunula na pomalú object/mpmath cestu | nie; nevznikol nijaký úplný shard ani pair fyzikálny payload, iba technické timeout receipts; BI/.15 ostáva na pôvodnom REVIEW | runner 378 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-135 smie zmeniť iba reprezentáciu podľa fázy: exact binary64 projekciu checkpointového HP-M1 pre obyčajné variantové F0/M3 coefficient solves a pôvodný decimal80 HP-M1 iba pre exact audit driver/holdout assembly; matice, rovnice, supporty, prahy a deadline ostávajú nezmenené | pred drahým behom explicitne oddeliť numerickú reprezentáciu pre štandardný solve od reprezentácie pre predregistrovanú exact boundary; vysokú presnosť nepropagovať do fáz, ktoré sa majú zhodovať s binary64 referenciou |

## Dodatok PF-121

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-121 | 379 / KMPC-135 BI/.15 official | compile/help a štvor-shard smoke prešli; binary64 `gamma0/accepted` shard prešiel za `1.656 s`, ale parent po `9 s` ukončil zostávajúce tri súbežné procesy; immutable failure SHA `35CD898E8331BCE3C2FC6D1EBA3144103942FF10A14E2FA32D59618E9C3A6D9E` | fázová numerická reprezentácia bola opravená, no audit worker stále sériovo kumuloval coefficient solve a decimal80 exact boundary; navyše dva drahé audit workery súťažili o CPU s accepted workermi v jednej vlne | iba jeden accepted support shard; nevznikol úplný variant ani pair payload a BI/.15 fyzikálny verdikt ostáva REVIEW z KMPC-131 | runner 379 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-136 smie zmeniť iba procesnú segmentáciu na prvú vlnu štyroch binary64 coefficient shardov a druhú vlnu dvoch audit-only exact-boundary shardov, s hashovo overeným dočasným handoffom a jedným finálnym immutable raw; rovnice, matice, supporty, prahy, precision a limity sa nemenia | runtime rozpočet testovať na reálnej drahej fáze, nie iba schema smoke; nekombinovať dve numericky odlišné drahé fázy v jednom worker deadline a neplánovať parent limit bez rezervy na CPU contention |

## Dodatok PF-122

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-122 | 380 / KMPC-136 BI/.15 official | compile/help/šesť-rolový smoke prešli; všetky štyri coefficient shardy prešli za `1.188–1.625 s`, ale oba exact workery zastali pred exact boundary na `KMPC-103 M1/fuel register order mismatch`; immutable failure SHA `3BA55975156DD8513C833F0CCDAD99F3091630556E509CB6B3E11F9563A95FAE` | canonical JSON publish s `sort_keys=true` zmenil insertion order fuel mapy na `U_f, delta_f`; frozen merge vyžaduje explicitne `delta_f, U_f`, hoci hodnoty a ich fingerprint ostali nezmenené | coefficient vlna je úplná technická evidencia, exact fyzika sa nespustila a pair payload nevznikol; BI/.15 verdikt ostáva REVIEW | runner 380 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-137 smie v tenkom identity wrapperi pred frozen merge obnoviť iba explicitné poradie fuel ownerov `delta_f, U_f`; KMPC-136 base, handoff hodnoty, hashe, rovnice, matice, prahy, precision a limity ostávajú nezmenené | každá serializovaná ordered registry musí po načítaní obnoviť poradie z frozen owner contractu, nie z JSON object order; smoke musí obsahovať reálne roundtrip poradie, nielen kanonický hash fixture |

## Dodatok PF-123

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-123 | 381 / KMPC-137 BI/.15 official | compile/help a fuel-order roundtrip smoke prešli; všetky štyri coefficient shardy prešli za `1.313–1.640 s`, oba exact workery však stage wall timeout ukončil bez payloadu; immutable failure SHA `213F2B2E2516BBD4FC5A14C52D5750FD32DDECC727578DFDE04F0FD58A42E72A` | poradie aj dvojvlnová architektúra sú opravené, ale frozen 80-dps exact solver je mimo aktuálneho runtime kontraktu: autoritatívny KMPC-112 ten istý exact mechanizmus dokončil za `34.86 s` s explicitným limitom `45 s`, nie `4.8 s` | štyri coefficient shardy sú úplné, no oba distinct exact variantové systémy ostali bez výsledku; pair fyzikálny payload nevznikol a BI/.15 ostáva REVIEW | runner 381 je `DO_NOT_RUN_AUDIT_TECHNICAL`; ďalší výpočet je zablokovaný do explicitnej metodickej voľby medzi osobitným exact limitom, novým auditovaným rýchlym solverom alebo predregistrovaným checkpointovaným rozkladom; žiadny automatický KMPC-138 | pred zmrazením runtime limitu kalibrovať drahú autoritatívnu fázu podľa historického receipt času; schema smoke nemôže dokazovať realizovateľnosť exact numeriky a technický limit nesmie potichu meniť fyzikálny verdikt |

## Dodatok PF-124

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-124 | 382 / KMPC-138 BI/.15 local-45 official | compile/help/smoke prešli; všetky štyri coefficient shardy prešli za `1.233–1.719 s`, oba exact child procesy však okamžite skončili na `ValueError: KMPC-128 runtime must be finite and in (0, 4.8]`; immutable failure SHA `E5B8D5AE2E8DEDBB7B3A0682D08BA8336AC6124BE3DFF5D352C1E99B7737CC83` | runner zmrazil nový exact cap, ale inherited `scientific._make_deadline` stále obsahuje globálny KMPC-128 guard `≤4.8`; lokálna používateľom povolená výnimka nebola routovaná k exact worker role | exact checkpoint ani exact boundary sa nespustili; iba 4/4 coefficient evidencia, bez pair payloadu a bez zmeny BI REVIEW | runner 382 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-139 smie iba v exact child role dočasne nahradiť deadline factory factorym povoľujúcim presne `45.0 s`, po volaní obnoviť pôvodného ownera a publikovať owner-lifecycle checks; coefficient workery ostávajú byte-semantic `≤4.8 s` | každá lokálna runtime výnimka musí byť routovaná cez všetky inherited validation vrstvy a smoke musí overiť nielen CLI hodnotu, ale aj process-local deadline owner lifecycle |

## Dodatok PF-125

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-125 | 383 / KMPC-139 BI/.15 local-45 official | compile/help/smoke prešli; 4/4 coefficient workery aj 2/2 exact workery skončili `returncode=0`, no parent merge následne skončil na `KeyError('contract_guard')`; immutable failure SHA `FBACDAB50EAC1D7ADB38104560F04806252E5A2DD19E605289A33B7E35FC334B` | dvojvlnový coefficient payload publikuje identický kontrakt pod novším názvom `successor_contract_guard`, kým zdedený KMPC-131 parent agregátor ešte číta historický alias `contract_guard`; schema smoke pred drahou vlnou nevolal produkčný agregátor nad realistickou fixture | áno; receipt uchoval všetkých šesť úspešných worker payloadov, exact `gamma0` aj `af0` majú `technical_pass=true` a `exact_boundary_pass=true`, ale parent pair payload nevznikol, preto BI/.15 zatiaľ nemení REVIEW | runner 383 je `DO_NOT_RUN_AUDIT_TECHNICAL`; KMPC-140 smie iba hashovo načítať immutable PF-125 receipt, overiť presnú 4+2 množinu a úspešné identity, do kópií coefficient payloadov doplniť `contract_guard := successor_contract_guard` a bez solverových/worker volaní zopakovať parent merge | produkčný aggregate smoke musí pred drahými workermi overiť úplnú vstupnú schému realistickej fixture; pri premenovaní zdieľaného poľa musí byť starý alias explicitne zachovaný alebo versionovane adaptovaný |

## Dodatok PF-126

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-126 | 384 / KMPC-140 read-only parent aggregate | compile/help/smoke aj official prešli; parent raw SHA `DF45DF6A937177A84832826400725553D5A0EADD104981E8F3992DC3FCC1638F`, runtime `0.109 s`, nulové worker/solver volania, no oba exact evidence bloky dostali false iba na `original_false_set_exact` | KMPC-136 hardcoded podmienka vyžaduje pôvodnú false množinu presne `['M3_driver','M3_independent_00_0i_holdout']`; po same-matrix refinement však v oboch variantoch holdout už prešiel a jediný pôvodný false bol `M3_driver`, takže úspešný exact driver aj úspešné exact potvrdenie holdoutu boli odmietnuté rovnostnou podmienkou | nie v KMPC-140; iba read-only agregácia už vykonanej fyziky. Exact driver/holdout hodnoty sú PASS, ale false-negative kompozícia ponecháva BI/.15 bez nového verdiktu | runner 384 je `DO_NOT_RUN_AUDIT_LOGIC_FALSE_NEGATIVE`; KMPC-141 smie hashovo načítať raw KMPC-140 a nahradiť iba rovnostný predikát korektným fail-closed predikátom: pôvodná false množina je neprázdna podmnožina deklarovaného supersession scope, každý pôvodný false je exact uzavretý a pôvodne PASS holdout je exact potvrdený; potom smie prepočítať iba odvodené pass/candidate polia | supersession kompozícia nesmie vyžadovať, aby všetky opraviteľné brány pred exact krokom zlyhali; musí zakázať false mimo scope a vyžadovať exact dôkaz pre pôvodné false aj konzistenciu už prechádzajúcich brán |

## Dodatok PF-127

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-127 | 375 / KMPC-131 NID/.05 four-shard smoke | compile a help prešli; všetky štyri smoke children skončili exit `2` na `RuntimeError: nominal identity mismatch: RUN_KMPC_053_P5_3G7_NID_SUPPORT_CLOSURE.json`; parent register bol `0/4`, bez rawu | frozen loader porovnal celý historický identity objekt s trojpoľovým objektom; KMPC-053 má správne `mode=NID`, `k=0.05`, `variant=nominal`, ale navyše legitímne frozen polia `M1_depth=7`, `candidate_support=[0,5]`, `audit_support=[0,7]` | nie; smoke iba načítal a hashovo overoval schému, `physics_executed=false`, nevznikol solver ani success/failure raw a NID/.05 ostáva iba pri historickom nominal PASS | runner 375 ostáva autoritatívny pre už prejdené identity, ale kombinácia `NID/.05` je `DO_NOT_RUN_WITH_LEGACY_IDENTITY_EQUALITY`; KMPC-142 smie pridať iba fail-closed schema adapter vyžadujúci presný šesťpoľový KMPC-053 identity objekt a potom znovu použiť byteovo nezmenenú KMPC-131 shard/aggregate fyziku | pri historických versionovaných schémach neporovnávať objekt s redukovanou mapou bez schema vetvy; pre každú schému zmraziť presnú povolenú množinu identity polí a smoke musí prejsť všetkých 15 nominal referencií alebo aspoň cielene každý odlišný schema typ |

## Dodatok PF-128

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-128 | 388 / KMPC-144 NID/.15 af0/audit-only refinement | compile/help/smoke/official prešli; af0 refined driver `1.3514e-16`, oba logical atoms a všetky fyzikálne brány true, ale pair ostal REVIEW pre dve false parity checks; raw SHA `7288ADE2BBC876D5F26677186ACF37BD3FE6B6DC439458C90A640B1C8FD103EB` | parent porovnal restored in-memory integer power keys s JSON string keys pri af0 accepted a celý gamma0 objekt vrátane nondeterministického runtime a nového true contract checku; JSON-semantická af0 accepted projekcia a vedecká gamma0 projekcia sú presne zhodné | áno; cielený af0 audit refinement aj všetky štyri shardy sú úplné a fyzikálne PASS, ale predregistrovaná chybná parity kompozícia zatiaľ blokuje nový af0 verdikt | runner 388 je `DO_NOT_RUN_AUDIT_LOGIC_FALSE_NEGATIVE`; KMPC-145 smie iba read-only načítať hashovo zmrazený raw, overiť presnú false množinu, vykonať dve explicitné semantické parity projekcie a zmeniť len odvodené parity/pair/candidate polia s protected-snapshot hashom; žiadne workery ani solvery | parity medzi in-memory a JSON artefaktom musí najprv kanonizovať key typy; runtime a nové pravdivé provenance checks nesmú byť súčasťou vedeckého snapshotu, ale ich povolené vylúčenie musí byť explicitné a fail-closed |

## Dodatok PF-129

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-129 | 390 / KMPC-146 NIV/.15 four-shard multi-rank refinement | compile/help/smoke/official prešli; všetky štyri rank-104/130 refined drivery klesli na `1.63e-16…2.14e-16`, oba logical atoms a všetky fyzikálne brány true, ale parent vydal REVIEW pre štyri false `f0_exact_predecessor_parity`; raw SHA `BA595163C3A2E1D464558B035FE478A16E36678FA215C46B124E4062DC77227E` | parent porovnal živé F0 stromy s integer power keys s JSON-načítaným predecessorom so string keys pred spoločnou publish kanonizáciou; v immutable JSON sú všetky štyri F0 projekcie exact zhodné | áno; štyri cielené M3 corrections aj celý inherited fyzikálny pipeline sú úplné a PASS, no chybná odvodená parity kompozícia zatiaľ blokuje nový NIV verdikt | runner 390 je `DO_NOT_RUN_AUDIT_LOGIC_FALSE_NEGATIVE`; KMPC-147 smie iba read-only hashovo načítať oba rawy, overiť presnú false štvoricu, JSON-semantic parity a protected snapshot a zmeniť len štyri parity/pair/candidate polia; žiadne workery ani solvery | parity živého objektu voči JSON artefaktu sa musí vykonať až po jednej explicitnej kanonizácii; smoke musí obsahovať integer-key verzus string-key fixture pre každý chránený subtree |

## Dodatok PF-130

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-130 | `.q1r7_acquire.ps1` / Q1R7-V1 | O1 vrátil HTTP `403`; body adapter skončil na `[System.Byte]` bez metódy `AsMemory`, potom proces skončil na chýbajúcej vlastnosti `.Count`; journal283C SHA `C104472F...D7BD2F` zachoval O1, commit receipt nevznikol | pozorovaný prvý fail je volanie `AsMemory` nad runtime prijímačom `[System.Byte]`; presná producer/scalarization/binder príčina zostáva otvorená do lokálneho nonnetwork fixture testu. Druhá vetva nemala fail-closed normalizovaný array-shape kontrakt pred `.Count`; presná null/scalar cesta zostáva iba pravdepodobná | nie; spotrebovala sa jedna source operácia, ale source universe, S0–S12 a fyzika Q1R7 sa nevyhodnotili | V1 transakcia je immutable `TECHNICAL_FAILURE_NO_PHYSICAL_INFERENCE`; Q1R7 zostáva `REVIEW_TECHNICAL_UNRESOLVED`, `historical_packages_total=1`, `consecutive_technical_failures=1/10`; O1 je spotrebované `1/6`, O2–O6 čakajú na versioned successor, nejde o Q1R8 handoff | pred sieťou overiť skutočné PowerShell-7 read API, HTTP 403/empty-body vetvu a zero/one/multiple array shape; successor musí hashovo viazať 283/283C/284, zachovať kumulatívne `1/6` a `1/10`, nereštartovať O1 bez osobitnej preregistrácie a prejsť nezávislým statickým auditom |

## Dodatok PF-131

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-131 | `.q1r7_v2_acquire.ps1` / Q1R7-V2 | O2 skončilo HTTP `200`, `636` bytes, SHA `A88DBC4877FAB415228D3BA6B98B20A3BF5E73D954E3FDFBE3B2163D4C4B2D5A`; O3 po približne `30.035 s` skončilo `status=-1`, bez body, s `The operation was canceled`; journal285C SHA `4A8002A...DE7F` ostal bez commit receiptu | frozen per-request response-header guard `CancelAfter(30 s)` zrušil O3 wait pred zaznamenaním final response statusu. Presná externá príčina nedokončenia — remote server, sieť alebo host scheduling — nie je z immutable dôkazu určená; nejde o dôkaz absencie arXiv zhody | nie; O2 binding ostal iba v pamäti a nebol publikovaný, O3 neposkytlo final status ani body, S0–S12 a fyzika sa nevyhodnotili | V2 transakcia je immutable `TECHNICAL_FAILURE_NO_PHYSICAL_INFERENCE`; `historical_packages_total=2`, `consecutive_technical_failures=2/10`, cumulative source operations `3/6`; O4–O6 sú nespotrebované a blokované; exact V2 rerun je zakázaný a nástupca tu nie je zvolený | každý prípadný ďalší krok musí hashovo viazať 285/285C/286 a procesné logy, zdediť `3/6` a `2/10`, nesmie znovu použiť nepublikovaný O2 binding ani implicitne meniť timeout, query, retry alebo cap a pred vykonaním musí prejsť samostatnou predregistráciou a nezávislým auditom |

## Dodatok PF-132

| ID | Skript/beh | Kategória a symptóm | Koreňová príčina | Vykonaná fyzika? | Stav a nástupca | Povinná prevencia |
|---|---|---|---|---|---|---|
| PF-132 | `291S_B6B2_10_Q1R1_V3_CANONICAL_HOST_SOURCE_ARCHIVE_ACQUISITION.ps1` / jediný offline SelfTest | `80/83`; tri syntakticky platné single-Location fixtures skončili `Cannot overwrite variable Host because it is read-only or constant`; exit `11`, fixture-set SHA `540907D5...BE2931` | PowerShell identifikátory nie sú case-sensitive, takže lokálne priradenie `$host = $resolved.IdnHost...` cieli read-only automatickú premennú `$Host`; host comparisons a normalization sú nedosiahnuteľné pre každú single hodnotu, ktorá prejde skoršími URI guardmi | nie; SelfTest mal network `0`, live writes `0`, Python `0`; request, archive parser a fyzika sa nespustili | frozen 291S SHA `62C82F...BFEC` je `DO_NOT_RUN_TECHNICAL`, SelfTest `1/1` bez rerunu; V3 request ostáva `0/1`, cumulative source operations `3`, packages `2`, counter `2/10`; iba versioned successor smie premenovať `$host` a dve referencie na nekolízny názov | pred freeze urobiť case-insensitive statickú kontrolu všetkých priraďovaných mien proti PowerShell automatic/preference variables, aliases a keywords; zachovať tri zlyhané fixtures ako povinnú regresiu |
