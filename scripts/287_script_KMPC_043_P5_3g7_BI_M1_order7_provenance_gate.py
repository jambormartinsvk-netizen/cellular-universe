"""Bounded runner for KMPC-043 BI M1 order-7 provenance gate."""
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_043_P5_3G7_BI_M1_ORDER7_PROVENANCE_GATE.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_043_P5_3G7_BI_M1_ORDER7_PROVENANCE_GATE_TECHNICAL_FAILURE.json"
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-043 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-043 exact pre-import source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-043 immutable prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-043 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC043_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-043 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded KMPC-043 BI M1 order-7 provenance only: order-5 "
            "regression, 121x99 full-power system and 18 holdouts; no support solve."
        )
    )
...
def main() -> int:
    ...
    try:
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-043 payload source hash mismatch")
            ...
            json.dumps(
                {
                    ...
                    "driver_pass": safe["driver_and_initial_full_power"]["pass"],
                    "holdout_pass": safe["holdout_full_power"]["pass"],
