...
- `337_script_KMPC_093_P5_3g7_C2_BI_k0p15_high_precision_M1_reassembly.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-096, compile/help a nové matematické
  smoke fixtures prešli, ale zdedený attribution owner checker nerozpoznal
  legitímny outer M1 overlay. Official sa nespustil a nevznikol raw.
  Povolený je iba KMPC-094 outer-aware owner successor; frozen HP-M1 modul
  a všetka výpočtová matematika sa nesmú meniť.

# 338 / KMPC-094

- `338_script_KMPC_094_P5_3g7_C2_BI_k0p15_high_precision_M1_attribution_owner_successor.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-097, smoke prešiel, no official
  unscaled `mpmath.qr_solve` označil reduced `121×98` systém numericky
  singulárny. Success raw nevznikol. Povolený je iba KMPC-095 successor so
  stĺpcovou ekvilibráciou; riadkové váhy, rovnice, vstupy a prahy sa nesmú
  meniť.

# 339 / KMPC-095

- `339_script_KMPC_095_P5_3g7_C2_BI_k0p15_high_precision_M1_column_equilibrated.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-098, zlyhal iba prehnane prísny a
  stringovo krehký scale fixture pred official. Povolený je iba KMPC-096
  fixture-only successor; column-equilibrated solver, rovnice, support,
  background a prahy zostávajú byteovo nezmenené.

# 340 / KMPC-096

- `340_script_KMPC_096_P5_3g7_C2_BI_k0p15_high_precision_M1_scale_fixture_successor.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-099, smoke prešiel, ale official
  column-equilibrated QR opäť hlásil numerical singularity. Raw nevznikol.
  Povolený je iba diagnostic-only KMPC-097 matrix provenance; ďalšia slepá
  výmena solvera nie je povolená.

# 341 / KMPC-097

- `341_script_KMPC_097_P5_3g7_C2_BI_k0p15_HP_M1_matrix_provenance_diagnostic.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-101, matrix provenance a diagnostický
  bridge prebehli, ale V1 potom nahradil 13-stavový combined register iba
  11 M1 stavmi a atribúcia skončila `KeyError: delta_f`. Failure SHA
  `9B1B1031...2F19E5`. Povolený je iba KMPC-098 register-merge successor;
  V5 matrix diagnostika, bridge, rovnice, support a prahy sa nemenia.

# 342 / KMPC-098

- `342_script_KMPC_098_P5_3g7_C2_BI_k0p15_HP_M1_combined_register_successor.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-102, 13-state merge prešiel, ale
  downstream KMPC-088 atribúcia správne nereprodukovala starý KMPC-087
  residual po diagnostickej zmene M1. Failure SHA `3CD0C73D...9CBBD6`.
  KMPC-099 musí byť standalone M1 matrix-provenance rez; stará atribučná
  brána ani jej tolerancie sa nesmú meniť.

# 343 / KMPC-099

- `343_script_KMPC_099_P5_3g7_C2_BI_k0p15_HP_M1_standalone_matrix_provenance.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; PF-103, standalone diagnostika sa
  dokončila a raw SHA `93780C85...E96ECD9` bol exclusive publikovaný, ale
  následný terminálny summary vyžadoval chýbajúci legacy `atom_id` a proces
  vrátil nonzero. KMPC-100 smie iba read-only validovať tento raw a vydať
  summary-compatible receipt; matrix výpočet sa nesmie opakovať.

# 345 / KMPC-101

- `345_script_KMPC_101_P5_3g7_C2_BI_k0p15_native_HP_M1_CPQR.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-104, compile/help/smoke prešli, ale
  prvý official príkaz použil iba basename v `--output`, nie canonical cestu
  `scripts/results/k_mpc_005/...`. Zlyhanie bolo v `guarded_import` pred M1
  assembly a CPQR solve; failure SHA `378A4FC7...A119CA`. KMPC-102 smie iba
  routing successor nad byteovo nezmeneným V9 a musí použiť plnú relatívnu
  canonical output cestu.

# 347 / KMPC-103

- `347_script_KMPC_103_P5_3g7_C2_BI_k0p15_HP_M1_downstream_insertion.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-105, compile prešiel, ale help/smoke sa
  zastavili pred CLI parserom, pretože import runnera 346 kvôli hash mapám
  vykonal jeho top-level `configure(KMPC-102)` a druhá konfigurácia bola
  fail-closed odmietnutá. Bez raw a bez fyziky. KMPC-104 smie iba staticky
  načítať literal mapy cez SHA-guarded AST; V11 sa nemení.

# 348 / KMPC-104

- `348_script_KMPC_104_P5_3g7_C2_BI_k0p15_HP_M1_downstream_AST_contract_successor.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-106, AST loader a všetky smoke checks
  prešli, ale V11 payload niesol frozen `run_id=KMPC-103`, nie KMPC-104.
  Official sa nespustil. KMPC-105 smie pridať iba identity-only V12 wrapper;
  V11 výpočet, metóda a prahy sa nemenia.

# 349 / KMPC-105

- `349_script_KMPC_105_P5_3g7_C2_BI_k0p15_HP_M1_downstream_identity_successor.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-107, compile/help/smoke prešli, ale
  monolit CPQR + accepted/audit support + exact boundary prekročil interných
  45 s. Failure SHA `DAF1A456...239EC3`. Nástupca musí výpočet segmentovať
  checkpointom; timeout ani fyzikálne prahy sa nezvyšujú.

# 350 / KMPC-106

- `350_script_KMPC_106_P5_3g7_C2_BI_k0p15_HP_M1_support_checkpoint.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-108, compile prešiel, ale help/smoke
  odmietli `ast.literal_eval(dict(...))` ešte pred CLI a fyzikou. KMPC-107
  smie použiť pinned posledný priamy literal ancestor a byteovo nezmenený
  V13 checkpoint.

# 351 / KMPC-107

- `351_script_KMPC_107_P5_3g7_C2_BI_k0p15_HP_M1_support_checkpoint_routing_successor.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-109, checkpoint prefix dobehol, ale
  publish odmietol diagnostický scalar `mpmath.mpf`. Failure SHA
  `ADB8D2A1...92F89C`. KMPC-108 smie zmeniť iba JSON reprezentáciu `mpf` na
  lossless decimal90 a reportovať všetky cesty.

# 352 / KMPC-108

- `352_script_KMPC_108_P5_3g7_C2_BI_k0p15_HP_M1_support_checkpoint_JSON_successor.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `683D867D...9D995` a
  terminálny summary boli publikované, potom host shell vrátil external
  timeout 124 (`PF-110`). KMPC-109 smie iba read-only overiť raw; checkpoint
  výpočet sa nesmie opakovať v živom strome.

# 353 / KMPC-109

- `353_script_KMPC_109_P5_3g7_C2_BI_k0p15_HP_M1_support_checkpoint_read_only_receipt.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; read-only receipt exit 0, raw SHA
  `21EF9A9B...28118F9`. Exact resume je dovolený iba novým predregistrovaným
  runnerom nad presnými SHA KMPC-108/109.

# 354 / KMPC-110

- `354_script_KMPC_110_P5_3g7_C2_BI_k0p15_HP_M1_checkpoint_exact_resume.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-111, compile/help prešli, ale smoke
  odovzdal po `sort_keys=True` alfabeticky načítaný M1 dict order-sensitive
  deserializátoru. Exact výpočet nebežal. KMPC-111 smie iba obnoviť M1/fuel
  mapy podľa hashovaných checkpointových order listov a delegovať V17.

# 355 / KMPC-111

- `355_script_KMPC_111_P5_3g7_C2_BI_k0p15_HP_M1_checkpoint_exact_resume_order_successor.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-112, compile/help/smoke prešli a
  order overlay fungoval, ale official porovnal živé integer-key/tuple typy
  s publikovanými JSON string-key/list typmi pred spoločnou kanonizáciou.
  Failure SHA `1ADCB30A...BD95E40`; in-memory exact výsledok nemá verdikt.
  KMPC-112 smie iba kanonizovať obe strany parity podľa stabilného publish
  kontraktu, bez zmeny výpočtu, checkpointu, prahov alebo runtime.

# 356 / KMPC-112

- `356_script_KMPC_112_P5_3g7_C2_BI_k0p15_HP_M1_checkpoint_exact_resume_JSON_parity_successor.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; compile/help/smoke/official exit 0,
  raw SHA `FAF52256...A6507A1`. Interný audit dokument 179 prijal scoped
  BI/k=.15 PASS. Immutable výsledok sa nesmie prepisovať ani opakovať.

# 357 / KMPC-113

- `357_script_KMPC_113_P5_3g7_C2_NID_k0p005_nominal.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `DD5B3075...3B533`.
  Nominal tail-only REVIEW bol spotrebovaný následníkom KMPC-114/115;
  immutable výsledok sa nesmie prepisovať ani opakovať.

# 358 / KMPC-114

- `358_script_KMPC_114_P5_3g7_C2_NID_k0p005_support_07_checkpoint.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; verdict-free checkpoint raw SHA
  `339FD13B...B35195`. PF-113 patrí prvému chybnému operátorskému smoke CLI
  volaniu, nie runneru; správny smoke a official checkpoint prešli. Cieľ sa
  nesmie prepisovať ani opakovať.

# 359 / KMPC-115

- `359_script_KMPC_115_P5_3g7_C2_NID_k0p005_support_07_09_checkpoint_resume.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `7D7B9BC1...4E0851`.
  Interný audit dokument 183 prijal scoped NID/k=.005 PASS. Immutable
  výsledok sa nesmie prepisovať ani opakovať.

# 360 / KMPC-116

- `360_script_KMPC_116_P5_3g7_C2_NID_k0p15_nominal.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `0965E3D1...8E25AB`.
  Jediný core numerical REVIEW bol spotrebovaný same-matrix nástupcom
  KMPC-117; immutable výsledok sa nesmie prepisovať ani opakovať.

# 361 / KMPC-117

- `361_script_KMPC_117_P5_3g7_C2_NID_k0p15_same_matrix_refinement.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `F9BE1AC9...E6C7C4`.
  Interný audit dokument 186 prijal scoped NID/k=.15 PASS a uzavrel celý
  NID mód. Immutable výsledok sa nesmie prepisovať ani opakovať.

# 362 / KMPC-118

- `362_script_KMPC_118_P5_3g7_C2_NIV_k0p005_nominal.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `FDB2DF9C...73B78F`.
  Tail-only REVIEW bol spotrebovaný checkpoint/resume nástupcami
  KMPC-119/120; immutable výsledok sa nesmie prepisovať ani opakovať.

# 363 / KMPC-119

- `363_script_KMPC_119_P5_3g7_C2_NIV_k0p005_support_06_checkpoint.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; verdict-free checkpoint raw SHA
  `0E87C19C...4ECEE9`. Nemá fyzikálny verdikt a bol spotrebovaný exact-hash
  resume KMPC-120; cieľ sa nesmie prepisovať ani opakovať.

# 364 / KMPC-120

- `364_script_KMPC_120_P5_3g7_C2_NIV_k0p005_support_06_08_checkpoint_resume.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `D6350636...AB136`.
  Interný audit dokument 190 prijal scoped NIV/k=.005 PASS. Immutable
  výsledok sa nesmie prepisovať ani opakovať.

# 365 / KMPC-121

- `365_script_KMPC_121_P5_3g7_C2_NIV_k0p15_nominal.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `8E5E8107...C0BEE`.
  Core+tail REVIEW spotrebovali KMPC-122/126; immutable výsledok sa nesmie
  prepisovať ani opakovať.

# 366 / KMPC-122

- `366_script_KMPC_122_P5_3g7_C2_NIV_k0p15_same_matrix_refinement.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `BAC68E1D...69419`.
  Nominal core sa uzavrel, tail-only REVIEW spotreboval KMPC-126; výsledok
  sa nesmie prepisovať ani opakovať.

# 367 / KMPC-123

- `367_script_KMPC_123_P5_3g7_C2_NIV_k0p15_support_06_checkpoint.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS / CHECKPOINT_INCOMPLETE`; raw SHA
  `D3B31093...98DC8`, `checkpoint_complete=false`. Bez fyzikálneho
  verdiktu; cieľ sa nesmie prepisovať ani použiť na resume.

# 368 / KMPC-124

- `368_script_KMPC_124_P5_3g7_C2_NIV_k0p15_support_06_08_checkpoint_resume.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-114, smoke exit 2 pred fyzikou pre
  incomplete KMPC-123 checkpoint; success raw nevznikol.

# 369 / KMPC-125

- `369_script_KMPC_125_P5_3g7_C2_NIV_k0p15_support_06_08_same_matrix_refinement.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-115 post-processing KeyError po
  rank-104-only refinement; failure raw SHA `1ED339AE...A31D0`, bez
  fyzikálneho verdiktu.

# 370 / KMPC-126

- `370_script_KMPC_126_P5_3g7_C2_NIV_k0p15_support_06_08_multi_rank_refinement.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA `1D46AFD1...B8AA0`.
  Interný audit 197 prijal scoped NIV/.15 PASS a uzavrel NIV mód;
  immutable výsledok sa nesmie prepisovať ani opakovať.

# 371 / KMPC-127

- `371_script_KMPC_127_P5_3g7_C2_authoritative_atom_aggregate.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; read-only raw SHA
  `CA33E416...0D247F`. Interný audit 199 uzavrel C2 aggregate a odblokoval
  C3 bez zmeny K4 skóre; immutable výsledok sa nesmie prepisovať.

# 372 / KMPC-128

- `372_script_KMPC_128_P5_3g7_C3_zero_variant_pair.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-117, prvý AD/.005 official prekročil
  spoločný `4.8 s` deadline počas `af0` po dokončení `gamma0`. Immutable
  failure raw SHA `E974DEE1...5E3D9`; bez fyzikálneho verdiktu. Nástupca
  KMPC-129 smie zmeniť iba procesnú granularitu, nie rovnice ani prahy.

# 373 / KMPC-129

- `373_script_KMPC_129_P5_3g7_C3_parallel_zero_variant_pair.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-118, oba paralelné smoke workery boli
  falošne zamietnuté pre check `physics_executed=False` vložený do
  `all(checks.values())`. Fyzika sa nespustila a raw nevznikol. KMPC-130
  smie opraviť iba význam smoke booleanu a run identity wrapper.

# 374 / KMPC-130

- `374_script_KMPC_130_P5_3g7_C3_parallel_smoke_identity_successor.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-119, AD/.005 `gamma0` worker mal
  všetky brány true za `4.7960 s`, no `af0` prekročil vlastný `4.8 s`
  deadline. Failure raw SHA `01E9498E...FB5C6`, bez finálneho fyzikálneho
  verdiktu. KMPC-131 smie rozdeliť iba support procesy a parent merge.

# 375 / KMPC-131 — selektorová technická výnimka a immutable cieľ

- `375_script_KMPC_131_P5_3g7_C3_four_support_shards.py --mode NID --k 0.05`
  — `DO_NOT_RUN_WITH_LEGACY_IDENTITY_EQUALITY`; PF-127, four-shard smoke
  skončil `0/4` pred fyzikou, lebo loader porovnal správny šesťpoľový
  KMPC-053 identity objekt s redukovanou trojpoľovou mapou. Raw nevznikol a
  ostatné už prejdené identity runnera 375 sa týmto nezneplatňujú. KMPC-142
  smie pridať iba exact schema adapter pre frozen KMPC-053 identity; shard
  fyzika, support, rovnice, prahy a limity ostávajú nezmenené.

- `375_script_KMPC_131_P5_3g7_C3_four_support_shards.py --mode NIV --k 0.005`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA
  `9088E7D8470E3F4CD118025ECA266646883A76ED87BED69B3FA1DCCEBB0FD156`.
  Interný audit 233 prijal `PASS_C3_NIV_K0P005_3_OF_3`; immutable výsledok
  sa nesmie prepisovať ani opakovať.

- `375_script_KMPC_131_P5_3g7_C3_four_support_shards.py --mode NIV --k 0.05`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; raw SHA
  `9E8E7D0F22D471E3C806DDBF5B2B4E587B209A537D55F1A8EFE259AC4F9DEFDD`.
  Interný audit 235 prijal `PASS_C3_NIV_K0P05_3_OF_3`; immutable výsledok
  sa nesmie prepisovať ani opakovať.

- `375_script_KMPC_131_P5_3g7_C3_four_support_shards.py --mode NIV --k 0.15`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS / FIRST_REVIEW_STOP`; raw SHA
  `88DFD9AAD5F378CBD9F7E7D1AA9738C40855CA0EC9FD191A77C682D643A0CFE6`.
  Interný audit 237 prijal
  `REVIEW_C3_NIV_K0P15_MULTI_RANK_NUMERICAL_BOUNDARY`; oba nulové atómy
  ostávajú REVIEW. Immutable výsledok sa nesmie prepisovať ani opakovať a
  refinement/successor vyžaduje nový predregistrovaný artefakt.

# 378 / KMPC-134

- `378_script_KMPC_134_P5_3g7_C3_BI_k0p15_HP_M1_exact_resume.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-120, compile/help/smoke prešli, ale
  všetky štyri official workery prekročili `4.8 s`, pretože decimal80 HP-M1
  register vstúpil aj do obyčajného coefficient solve. Immutable failure raw
  SHA `F4BBC3CE...10A2`, bez fyzikálneho verdiktu. KMPC-135 smie oddeliť iba
  binary64 coefficient fázu od decimal80 exact-boundary fázy.

# 379 / KMPC-135

- `379_script_KMPC_135_P5_3g7_C3_BI_k0p15_phase_separated_exact_resume.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-121, binary64 `gamma0/accepted` shard
  prešiel za `1.656 s`, no parent po `9 s` ukončil tri zvyšné procesy, lebo
  audit shardy stále kumulovali coefficient solve a decimal80 exact boundary.
  Immutable failure SHA `35CD898E...A6D9E`, bez pair fyzikálneho verdiktu.
  KMPC-136 smie oddeliť iba coefficient a exact-boundary worker vlnu.

# 380 / KMPC-136

- `380_script_KMPC_136_P5_3g7_C3_BI_k0p15_two_wave_exact_resume.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-122, všetky štyri coefficient shardy
  prešli za `1.188–1.625 s`, oba exact workery však zastali pred výpočtom na
  JSON-zmenenom fuel owner poradí. Immutable failure SHA
  `3BA55975...A95FAE`, bez pair verdiktu. KMPC-137 smie obnoviť iba frozen
  poradie `delta_f, U_f` v tenkom wrapperi.

# 381 / KMPC-137

- `381_script_KMPC_137_P5_3g7_C3_BI_k0p15_fuel_order_roundtrip.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-123, všetky štyri coefficient shardy
  prešli za `1.313–1.640 s`, ale oba distinct 80-dps exact workery prekročili
  stage budget. Historical authority KMPC-112 potrebovala na ten istý exact
  mechanizmus `34.86 s` pri limite `45 s`. Immutable failure SHA
  `213F2B2E...2E72A`, bez pair verdiktu. Ďalší beh vyžaduje metodické
  rozhodnutie; automatický KMPC-138 je zakázaný.

# 382 / KMPC-138

- `382_script_KMPC_138_P5_3g7_C3_BI_k0p15_local_45s_exact_exception.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-124, 4/4 coefficient shardy prešli,
  ale oba exact children pred fyzikou odmietol inherited KMPC-128 guard
  `runtime≤4.8`. Immutable failure SHA `E5B8D5AE...37CC83`. KMPC-139 smie
  routovať používateľom schválených presne `45.0 s` iba process-local exact
  deadline overlayom s povinnou obnovou pôvodného ownera.

# 383 / KMPC-139

- `383_script_KMPC_139_P5_3g7_C3_BI_k0p15_local_exact_deadline_owner.py`
  — `DO_NOT_RUN_AUDIT_TECHNICAL`; PF-125, všetky štyri coefficient workery
  aj oba exact workery prešli, ale parent merge po fyzike zlyhal na
  historickom názve `contract_guard`, keď payload niesol tú istú hodnotu ako
  `successor_contract_guard`. Immutable failure SHA `FBACDAB50...334B`.
  KMPC-140 smie iba read-only adaptovať tento alias nad hashovo zmrazenými
  šiestimi payloadmi a spustiť parent merge bez workerov a solverov.

# 384 / KMPC-140

- `384_script_KMPC_140_P5_3g7_C3_BI_k0p15_read_only_aggregate.py`
  — `DO_NOT_RUN_AUDIT_LOGIC_FALSE_NEGATIVE`; read-only aggregate technicky
  prešiel za `0.109 s` bez workerov a solverov, ale inherited rovnostná
  podmienka odmietla pôvodnú false množinu `['M3_driver']`, lebo očakávala
  presne aj už prechádzajúci holdout. Raw SHA `DF45DF6A...1638F` je
  immutable REVIEW evidencia. KMPC-141 smie opraviť iba tento množinový
  supersession predikát a odvodené pass/candidate polia.

# 388 / KMPC-144

- `388_script_KMPC_144_P5_3g7_C3_NID_k0p15_af0_same_matrix_refinement.py`
  — `DO_NOT_RUN_AUDIT_LOGIC_FALSE_NEGATIVE`; PF-128, všetka fyzika vrátane
  af0 refined drivera `1.3514e-16` prešla, ale parent odmietol dve parity
  checks pre JSON key typy, runtime a nový true provenance check. Immutable
  raw SHA `7288ADE2...103EB` ostáva fyzikálnou evidenciou bez nového af0
  verdiktu. KMPC-145 smie iba read-only opraviť presne tieto dve parity
  projekcie a odvodené pair/candidate polia bez workerov a solverov.

# 390 / KMPC-146

- `390_script_KMPC_146_P5_3g7_C3_NIV_k0p15_multi_rank_refinement.py`
  — `DO_NOT_RUN_AUDIT_LOGIC_FALSE_NEGATIVE`; PF-129, všetky štyri
  rank-104/130 corrections aj fyzikálne brány prešli, ale parent porovnal
  živé integer power keys s predecessor JSON string keys a falošne odmietol
  štyri F0 parity checks. Immutable raw SHA
  `BA595163...77227E` sa nesmie prepisovať ani opakovať. KMPC-147 smie iba
  read-only kanonizovať a overiť tieto štyri parity projekcie a odvodené
  pass/candidate polia s nulou workerov a solverov.

# 391 / KMPC-147

- `391_script_KMPC_147_P5_3g7_C3_NIV_k0p15_read_only_f0_parity_correction.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; read-only official prešiel s
  `workers=solvers=physics=0`, protected snapshot exact a raw SHA
  `2780A8D652...56B16E`. Interný audit 241 prijal NIV `9/9` a globálne C3
  `45/45 logical PASS`. Immutable cieľ sa nesmie prepisovať ani opakovať.

# 392 / KMPC-148

- `392_script_KMPC_148_P5_3g7_C3_authoritative_logical_aggregate.py`
  — `DO_NOT_RUN_IMMUTABLE_TARGET_EXISTS`; jediný read-only official prešiel
  za `0.047 s` s `20/20` frozen vstupmi, exact registrom `45/45` a
  `workers=solvers=physics=matrices_built=0`. Immutable raw SHA
  `C493B102...78238` sa nesmie prepisovať ani opakovať. Interný audit 243
  prijal iba formálny C3 aggregate; K4 ostáva `60/100`.

# Q1R1-V3 / 291S

- `291S_B6B2_10_Q1R1_V3_CANONICAL_HOST_SOURCE_ARCHIVE_ACQUISITION.ps1`
  — `DO_NOT_RUN_TECHNICAL`; PF-132. Jediný offline SelfTest skončil
  `80/83`, exit `11`, pretože lokálna `$host` koliduje case-insensitive s
  read-only automatickou `$Host`. Network `0`, live writes `0`, request
  `0/1`; archive ani result nevznikli. Frozen SHA
  `62C82FE193DFA565EDF09E251BB80ABF679AA3EE6EA713287298A0245F1DBFEC`
  sa nesmie editovať ani spustiť znova. Povolený môže byť iba samostatne
  auditovaný versioned successor s rename na nekolízny identifikátor a s
  tromi pôvodnými Location regresnými fixtures.
