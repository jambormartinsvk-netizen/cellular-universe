"""Bounded runner for KMPC-045 GLOBAL_C1 / BI_SUPPORT_STEP_3."""
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_045_P5_3G7_BI_SUPPORT_STEP_3_05_07.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_045_P5_3G7_BI_SUPPORT_STEP_3_05_07_TECHNICAL_FAILURE.json"
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-045 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-045 exact pre-import source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-045 immutable prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-045 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC045_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-045 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded KMPC-045 BI support step 3: regression [0,3]/[0,5], "
            "candidate [0,5], audit [0,7], common 0..5, tail 6,7; no [0,9]."
        )
    )
...
def main() -> int:
    ...
    try:
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-045 payload source hash mismatch")
