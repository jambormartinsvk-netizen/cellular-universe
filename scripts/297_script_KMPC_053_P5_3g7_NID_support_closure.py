"""Bounded runner for KMPC-053 NID support closure.
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_053_P5_3G7_NID_SUPPORT_CLOSURE.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_053_P5_3G7_NID_SUPPORT_CLOSURE_TECHNICAL_FAILURE.json"
...
EXPECTED_SOURCE_HASHES = {
    ...
    "nid_support_closure.py": "0B9DA3BBF95FE14878DBD2604C7B8E9619BE9F94A4C147C4FAA79299DC165F5A",
}
EXPECTED_PREREQUISITES = {
    "RUN_KMPC_052_P5_3G7_NID_DEPTH7_NUMERICAL_BOUNDARY.json": "FDEE962EED16EDF459D7D8504833AB1206AEF1BFC8178A356A88A121CF196C4C",
    "RUN_KMPC_048_P5_3G7_NID_SUPPORT_STEP_2_05_07.json": "B4F320F5D850DCF78FD9EC2A5BDDEBDA87D590DA2988CF505FA7D5B25B49BF32",
}
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-053 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-053 exact source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-053 prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-053 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC053_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-053 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Bounded KMPC-053 NID support-[0,5] closure after depth-7 boundary audit.")
...
def main() -> int:
    ...
    try:
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-053 payload source hash mismatch")
        ...
        print(json.dumps({
            ...
            "reference_pass": safe["reference_pass"],
            "core_pass": safe["core_pass"],
            "common_pass": safe["common_pass"],
            "tail_pass": safe["tail_pass"],
