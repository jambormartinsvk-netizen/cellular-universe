...
Deväť rolí neznamená deväť súbežných agentov. Aktivujú sa podľa fázy a
...
| reviewer progresu a cieľa | `progress_goal_reviewer.toml` | read-only po uzavretom tasku vyhodnotiť informačný zisk, route/A3 posun, efektívnosť, duplicity a goal drift | editovať, spúšťať Python, prideľovať verdict/skóre/hĺbku, meniť scope alebo nahradiť autora/orchestrátora | klasifikácia progresu, drift alert a najmenší užitočný successor orchestrátorovi |
...
### 4.6 Post-task review progresu a cieľa

Po každom uzavretom tasku alebo výpočtovom atóme, ktorý priniesol či mal
priniesť nový dôkaz, zmenil blocker/next action alebo spotreboval technický
pokus, rola `progress_goal_reviewer` vykoná read-only kontrolu pred otvorením
ďalšej vedeckej úlohy. Granularitou je uzavretý task/atóm, nie každý príkaz,
hash, preflight fáza alebo nedokončený medzikrok.

Reviewer porovná objective a `DONE_WHEN` s immutable výstupmi a uvedie:

1. autoritatívny stav pred a po bez jeho prepisovania;
2. dotknutú route, gate, hĺbku a vplyv na cestu k A3;
3. skutočný informačný zisk a odstránené, zachované alebo nové vetvy;
4. pomer informačného zisku k počtu live artefaktov, auditov a behov;
5. duplicitu, opakovanie alebo odklon od aktuálneho blockeru;
6. najmenší ďalší krok, ktorý môže zmeniť rozhodovací stav.

Prvý posudok route môže založiť baseline. Následné posudky sú delta reviews:
okrem povinného bootstrapu a aktívneho kapsulu čítajú iba zmenené autoritatívne
záznamy a exact výstupy uzavretého tasku. Byte-identické kópie v auditnom
balíku sa pri manifest parity znovu celé neanalyzujú a reviewer neskenuje
repozitár rekurzívne. Ak bounded read-set nestačí, vráti presný
missing-evidence blocker namiesto rozšírenia scope.

Primárna klasifikácia je presne jedna z:
...
`GOAL_DRIFT_ALERT` sa vydá najmä pri skoku do inej route/hĺbky bez rozhodnutia
autora, opakovaní už uzavretého dôkazu, úprave frozen vstupov po výsledku,
rozvíjaní detailnej mikrofyziky pred feasibility, auditnom alebo súborovom
churne bez primeraného informačného zisku alebo práci mimo aktuálneho
blockeru. Alert uvedie presný odklon, posledný platný uzol a návratový krok.

Reviewer je poradný: neprideľuje `PASS/REVIEW/STOP`, skóre ani hĺbku,
nepovoľuje run a nemení dokumentáciu. Hlavný orchestrátor posudok prijme,
obmedzí alebo odmietne a iba on vykoná prípadnú autoritatívnu zmenu.
