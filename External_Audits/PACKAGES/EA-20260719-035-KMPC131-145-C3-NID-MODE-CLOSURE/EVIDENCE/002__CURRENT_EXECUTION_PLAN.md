...
| P5.3 | B1 PASS; C2 `10/10` a aggregate PASS; C3 autoritatívne `39/45` logical PASS | AD, CDI, BI aj NID sú `9/9`; zostáva šesť NIV nulových atómov, potom C3 aggregate; S-M mikrofyzická para ostáva otvorená |
...
Interný audit 231 uzavrel NID `9/9`; pred ďalším výpočtom sa vytvára a
odovzdáva externý NID mode-closure balík. Po jeho prijatí read-only preveriť
NIV/.005 nominal authority, support/depth, leading `j=-1`, nekolidujúci
output a runtime realizovateľnosť. C3 aggregate ostáva zakázaný do `45/45`.
