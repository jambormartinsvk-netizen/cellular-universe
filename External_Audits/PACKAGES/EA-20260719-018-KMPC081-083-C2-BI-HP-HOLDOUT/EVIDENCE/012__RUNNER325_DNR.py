"""80-dps independent-holdout runner for KMPC-081 C2 BI/k=.15.
...
from baseScripts.p5_general_synchronous import c2_bi_k0p15_high_precision_holdout as audit  # noqa: E402

if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-081 deterministic single-thread environment not active")

audit.configure(
    run_id="KMPC-081", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_081_P5_3G7_C2_BI_K0p15_HIGH_PRECISION_HOLDOUT_BOUNDARY.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-081",
        audit_module="baseScripts.p5_general_synchronous.c2_bi_k0p15_high_precision_holdout",
        aggregate_name="NOT_AVAILABLE_KMPC_081.json",
