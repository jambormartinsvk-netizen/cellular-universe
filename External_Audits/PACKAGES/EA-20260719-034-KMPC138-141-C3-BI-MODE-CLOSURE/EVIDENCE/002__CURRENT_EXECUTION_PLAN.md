...
K4-B2 limit. K4 score sa nemení.
...
| P5.3 | B1 PASS; C2 `10/10` a aggregate PASS; C3 autoritatívne `33/45` logical PASS | AD, CDI aj BI sú `9/9`; zostáva NID a NIV nulová coverage, potom C3 aggregate; S-M mikrofyzická para ostáva otvorená |
...
### C3-NID — nasledujúca koherentná mode jednotka

Najprv zapečatiť a odovzdať kompaktný externý auditný balík uzavretia BI.
Do prijatia jeho posudku sa nespúšťa ďalšia fyzika tejto vetvy. Potom
predregistrovať `NID/k=.005/gamma0+af0` až po kontrole nominal authority,
supportov, runtime realizovateľnosti a toho, či frozen KMPC-131 kontrakt možno
bezpečne znovu použiť. C3 aggregate ostáva zakázaný do `45/45`.
