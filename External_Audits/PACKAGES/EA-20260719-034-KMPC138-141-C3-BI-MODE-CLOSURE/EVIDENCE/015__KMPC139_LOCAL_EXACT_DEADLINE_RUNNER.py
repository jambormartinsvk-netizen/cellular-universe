"""CLI runner for KMPC-139 BI/.15 local exact deadline owner.
...
c3.RUN_ID = RUN_ID
_FROZEN_DEADLINE_FACTORY = c3.v8.scientific._make_deadline


def output_name(mode: str, k_mpc: float) -> str:
    if (mode, k_mpc) != ("BI", 0.15):
        raise ValueError("KMPC-139 is frozen to BI/k=0.15")
    return (
...
@contextmanager
def _local_exact_deadline_overlay():
    if c3.v8.scientific._make_deadline is not _FROZEN_DEADLINE_FACTORY:
        raise RuntimeError("KMPC-139 inherited deadline owner not pristine")
    diagnostic = {
        "local_45s_owner_active": False,
        "original_deadline_owner_restored": False,
    }

    def local_make_deadline(limit_seconds: float):
        if (
            not math.isfinite(limit_seconds)
            or limit_seconds != EXACT_MAX_RUNTIME_SECONDS
        ):
            raise ValueError("KMPC-139 local exact runtime must equal 45.0 s")
        started = time.monotonic()

        def deadline() -> None:
            if time.monotonic() - started > limit_seconds:
                raise TimeoutError("KMPC-139 local 45-s exact deadline exceeded")

        return started, deadline

    c3.v8.scientific._make_deadline = local_make_deadline
    diagnostic["local_45s_owner_active"] = (
        c3.v8.scientific._make_deadline is local_make_deadline
    )
    try:
        yield diagnostic
    finally:
        c3.v8.scientific._make_deadline = _FROZEN_DEADLINE_FACTORY
        diagnostic["original_deadline_owner_restored"] = (
            c3.v8.scientific._make_deadline is _FROZEN_DEADLINE_FACTORY
        )


def _deadline_overlay_smoke() -> dict[str, bool]:
    inherited_rejects_45 = False
    try:
        _FROZEN_DEADLINE_FACTORY(EXACT_MAX_RUNTIME_SECONDS)
    except ValueError:
        inherited_rejects_45 = True
    wrong_value_rejected = False
    with _local_exact_deadline_overlay() as diagnostic:
        _started, deadline = c3.v8.scientific._make_deadline(
            EXACT_MAX_RUNTIME_SECONDS
        )
        deadline()
        try:
            c3.v8.scientific._make_deadline(EXACT_MAX_RUNTIME_SECONDS - 1.0)
        except ValueError:
            wrong_value_rejected = True
        local_active = bool(diagnostic["local_45s_owner_active"])
    return {
        "inherited_owner_rejects_45s": inherited_rejects_45,
        "local_owner_accepts_exactly_45s": local_active,
        "local_owner_rejects_non_45s": wrong_value_rejected,
        "original_deadline_owner_restored": bool(
            diagnostic["original_deadline_owner_restored"]
        ),
    }
    ...
    for variant, level in c3.SHARDS:
        ...
        if not smoke and level == "audit":
            if handoff_dir is None:
                raise RuntimeError("KMPC-139 handoff directory missing")
            command.extend([
                "--handoff-output",
    ...
    for variant in VARIANT_CHOICES:
        ...
        if not smoke:
            if coefficient_payloads is None or handoff_dir is None:
                raise RuntimeError("KMPC-139 exact command handoff inputs missing")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.worker_exact_smoke_variant is not None:
        ...
        payload["checks"].update(_deadline_overlay_smoke())
    ...
    if args.worker_coefficient_shard is not None:
        ...
        if level == "audit":
            if args.handoff_output is None or not isinstance(handoff, dict):
                raise RuntimeError("KMPC-139 audit worker handoff output missing")
            ...
            raise RuntimeError("KMPC-139 accepted worker emitted audit handoff")
        else:
    ...
    if args.worker_exact_variant is not None:
        if (
            ...
            raise RuntimeError("KMPC-139 exact worker handoff arguments missing")
        with _local_exact_deadline_overlay() as deadline_owner:
        ...
        payload["technical_checks"]["local_45s_deadline_owner_active"] = bool(
            deadline_owner["local_45s_owner_active"]
        )
        payload["technical_checks"][
            "original_deadline_owner_restored"
        ] = bool(deadline_owner["original_deadline_owner_restored"])
    ...
    if args.max_runtime_seconds != COEFFICIENT_MAX_RUNTIME_SECONDS:
        raise ValueError("KMPC-139 coefficient limit must remain exactly 4.8 s")
    if args.exact_max_runtime_seconds != EXACT_MAX_RUNTIME_SECONDS:
        raise ValueError("KMPC-139 exact limit must remain exactly 45.0 s")
    ...
    try:
        with tempfile.TemporaryDirectory(
            prefix="KMPC139_HANDOFF_", dir=result_dir
        ) as temporary_name:
            ...
            if not _records_pass(coefficient_records, 4):
                raise RuntimeError("KMPC-139 coefficient wave incomplete")
            ...
            if remaining <= 0.0:
                raise TimeoutError("KMPC-139 no parent budget for exact wave")
            exact_records = _run_commands(
                _exact_commands(
                    script_path,
                    args.mode,
            ...
            if not _records_pass(exact_records, 2):
                raise RuntimeError("KMPC-139 exact-boundary wave incomplete")
        ...
        if not temporary_removed:
            raise RuntimeError("KMPC-139 temporary handoff directory not removed")
        ...
        if time.monotonic() - parent_started > PARENT_TOTAL_WALL_LIMIT_SECONDS:
            raise TimeoutError("KMPC-139 parent total wall limit exceeded")
