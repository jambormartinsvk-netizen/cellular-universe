"""Bounded runner for KMPC-055 NIV support step 2.
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_055_P5_3G7_NIV_SUPPORT_STEP_2_MINUS1_4_MINUS1_6.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_055_P5_3G7_NIV_SUPPORT_STEP_2_MINUS1_4_MINUS1_6_TECHNICAL_FAILURE.json"
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-055 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-055 exact pre-import source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-055 immutable prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-055 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC055_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-055 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Bounded KMPC-055 NIV support [-1,4] versus [-1,6] with M1 depth 6."
    )
...
def main() -> int:
    ...
    try:
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-055 payload source hash mismatch")
