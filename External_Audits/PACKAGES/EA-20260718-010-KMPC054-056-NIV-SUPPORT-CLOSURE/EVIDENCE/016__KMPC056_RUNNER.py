"""Bounded PF-076 owner-only successor for KMPC-056.
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_056_P5_3G7_NIV_SUPPORT_STEP_2_FINITE_OWNER_SUCCESSOR.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_056_P5_3G7_NIV_SUPPORT_STEP_2_FINITE_OWNER_SUCCESSOR_TECHNICAL_FAILURE.json"
...
EXPECTED_PREREQUISITES = {
    "RUN_KMPC_054_P5_3G7_NIV_C1_PRIMARY_EXTENDED_COVERAGE.json": "0CF322A7BA5964B78BBF9180B29FA8BBBE43A646ECEB05D444B6250568ECFB1E",
}
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-056 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-056 exact pre-import source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-056 immutable prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-056 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC056_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-056 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="KMPC-056 PF-076 finite-owner successor; unchanged NIV support step 2.")
    ...
    try:
        ...
        if args.audit:
            ...
            if requested.resolve() != CANONICAL_OUTPUT.resolve():
                raise ValueError("audit requires canonical KMPC-056 --output")
        progress["phase"] = "preimport_guard"
        preimport = _preimport_guard()
        progress["phase"] = "guarded_import"
        from baseScripts.p5_general_synchronous import niv_support_step2_v2_finite_owner as audit

        if audit.source_hashes() != EXPECTED_SOURCE_HASHES:
            raise RuntimeError("KMPC-056 post-import source hash mismatch")
        if args.smoke:
            progress["phase"] = "runner_smoke"
            runner = _runner_fixtures()
            if not all(runner.values()):
                raise RuntimeError("KMPC-056 runner fixture failed")
            progress["phase"] = "base_smoke"
            payload = audit.run_smoke(args.max_runtime_seconds, RESULT_DIR)
            if not payload["passed"]:
                raise RuntimeError("KMPC-056 base smoke failed")
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-056 payload source hash mismatch")
        progress["phase"] = "publish"
        _write_atomic_exclusive(CANONICAL_OUTPUT, payload)
        safe = _json_safe(payload)
        print(json.dumps({
            "run_id": safe["run_id"], "execution_status": safe["execution_status"],
            "candidate": safe["candidate_interpretation_not_verdict"],
            "owner_restored": safe["PF076_owner_overlay"]["owner_namespace_restored"],
            "regression_pass": safe["regression_against_KMPC054"]["pass"],
            "M1_depth6_pass": safe["M1_depth6_metadata"]["pass"],
            "core_pass": safe["core_pass"], "common_pass": safe["common_coefficient_pass"],
            "tail_pass": safe["pure_tail_pass"], "output": str(CANONICAL_OUTPUT),
        }, sort_keys=True), flush=True)
        return 0
    except Exception as exc:
        failure = {
            "run_id": "KMPC-056", "execution_status": "TECHNICAL_FAILURE_NO_PHYSICS_VERDICT",
            "authorship": {"theory_author": "Martin Jambor", "script_creator": "Codex (OpenAI)"},
            "phase": progress["phase"], "exception_type": type(exc).__name__, "message": str(exc),
