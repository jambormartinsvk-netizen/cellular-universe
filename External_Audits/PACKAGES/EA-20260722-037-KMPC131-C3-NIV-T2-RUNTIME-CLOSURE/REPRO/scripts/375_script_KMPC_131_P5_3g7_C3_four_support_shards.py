"""CLI runner for KMPC-131 four parallel C3 support shards.
...
from baseScripts.p5_general_synchronous import (
    c3_zero_variant_parallel_v3_support_shards as c3,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run one C3 pair through four bounded support workers."
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not _records_pass(records):
            raise RuntimeError("one or more KMPC-131 support workers failed")
