"""Runner for KMPC-057 ten-atom C2 Fourier coverage and aggregate.
...
AGGREGATE_NAME = "RUN_KMPC_057_P5_3G7_C2_FOURIER_COVERAGE_AGGREGATE.json"
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-057 requires exactly 4.8 runtime seconds")
...
def _preimport_guard(target: Path | None) -> dict[str, str]:
    if target is not None:
        ...
        if conflicts:
            raise FileExistsError(f"immutable KMPC-057 output conflict: {conflicts}")
    actual = {name: _sha256_file(BASE_DIR / name) for name in EXPECTED_SOURCE_HASHES}
    if actual != EXPECTED_SOURCE_HASHES:
        different = sorted(name for name, expected in EXPECTED_SOURCE_HASHES.items() if actual.get(name) != expected)
        raise RuntimeError(f"KMPC-057 exact source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-057 prerequisite hash mismatch: {name}")
    return actual


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC057_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-057 publish fixture")
    ...
    try:
        ...
        if fixture.exists():
            fixture.unlink()
    return {"publish_collision_rejected": collision_rejected,
            "publish_collision_preserved": preserved,
            "publish_temp_cleaned": not temporary.exists()}


def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="KMPC-057 C2 Fourier coverage: ten atoms plus aggregate.")
...
def main() -> int:
    ...
    try:
        ...
        if args.smoke:
            if args.mode is not None or args.k_mpc is not None or args.output is not None:
                raise ValueError("smoke forbids mode, k and output")
        elif args.atom:
            if args.mode is None or args.k_mpc is None or args.output is None:
                raise ValueError("atom requires mode, k and output")
        elif args.aggregate:
            if args.mode is not None or args.k_mpc is not None or args.output is None:
                raise ValueError("aggregate forbids mode/k and requires output")
        progress["phase"] = "guarded_import"
        from baseScripts.p5_general_synchronous import c2_fourier_coverage as audit

        if args.atom:
            expected_name = audit.atom_output_name(args.mode, args.k_mpc)
            target = RESULT_DIR / expected_name
        elif args.aggregate:
        ...
        if target is not None:
            ...
            if requested.resolve() != target.resolve():
                raise ValueError("output path differs from canonical KMPC-057 target")
        progress["phase"] = "preimport_guard"
        preimport = _preimport_guard(target)
        if audit.source_hashes() != EXPECTED_SOURCE_HASHES:
            raise RuntimeError("KMPC-057 post-import source hash mismatch")
        if args.smoke:
            ...
            if not payload["passed"] or not all(runner.values()):
                raise RuntimeError("KMPC-057 smoke failed")
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-057 payload source hash mismatch")
        progress["phase"] = "publish"
        _write_atomic_exclusive(target, payload)
        safe = _json_safe(payload)
        if args.atom:
            summary = {"run_id": safe["run_id"], "atom_id": safe["atom_id"],
                       "candidate": safe["candidate_interpretation_not_verdict"],
                       "M1_pass": safe["M1"]["pass"], "core_pass": safe["core_pass"],
                       "common_pass": safe["common_pass"], "tail_pass": safe["tail_pass"],
                       "background_pass": safe["background_guard"]["pass"], "output": str(target)}
        else:
            summary = {"run_id": safe["run_id"], "candidate": safe["candidate_interpretation_not_verdict"],
                       "observed_atoms": safe["matrix"]["observed_atoms"],
                       "all_atoms_pass": safe["all_atoms_pass"],
                       "background_spread_pass": safe["background_spread_pass"], "output": str(target)}
        print(json.dumps(summary, sort_keys=True), flush=True)
        return 0
    except Exception as exc:
        failure = {"run_id": "KMPC-057", "execution_status": "TECHNICAL_FAILURE_NO_PHYSICS_VERDICT",
        ...
        if target is not None:
            ...
            conflicts = [path for path in (target, failure_path, target.parent / f".tmp-{target.name}") if path.exists()]
            if not conflicts:
                _write_atomic_exclusive(failure_path, failure)
