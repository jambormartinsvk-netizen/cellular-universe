"""KMPC-066 AD/k=.15 support ladder [0,4] -> [0,6].
...
K_MPC = 0.15
VARIANT = "nominal"
OUTPUT_NAME = "RUN_KMPC_066_P5_3G7_C2_AD_K0p15_SUPPORT_04_06.json"
KMPC065_NAME = "RUN_KMPC_065_P5_3G7_C2_AD_K0p15_NOMINAL.json"
KMPC065_SHA256 = "987E467EA2F36EA8F061F665A33AE1F6DC9AB6E2EFE9FB710E23CE0C50171636"
KMPC065_CANDIDATE = "REVIEW_C2_AD_K0p15_SUPPORT_04_06_REQUIRED"
...
def atom_output_name(mode: str, k_mpc: float) -> str:
    if mode != MODE or k_mpc != K_MPC:
        raise ValueError("KMPC-066 permits only AD/k=.15")
    return OUTPUT_NAME
...
def source_hashes() -> dict[str, str]:
    ...
    hashes["c2_ad_k0p15_support_04_06.py"] = legacy.sha256_file(
        here / "c2_ad_k0p15_support_04_06.py"
    )
    return hashes


def _load_kmpc065(result_dir: Path) -> dict[str, object]:
    path = result_dir / KMPC065_NAME
    observed_hash = legacy.sha256_file(path)
    import json
    payload = json.loads(path.read_text(encoding="utf-8"))
    candidate = payload.get("candidate_interpretation_not_verdict")
    identity = payload.get("identity")
    if observed_hash != KMPC065_SHA256 or candidate != KMPC065_CANDIDATE:
        raise RuntimeError("immutable KMPC-065 prerequisite mismatch")
    if identity != {"mode": MODE, "k_Mpc_inverse": K_MPC, "variant": VARIANT}:
        raise RuntimeError("KMPC-065 identity mismatch")
    return {"file": KMPC065_NAME, "sha256": observed_hash, "candidate": candidate}
...
def run_smoke(max_runtime_seconds: float, result_dir: Path) -> dict[str, object]:
    ...
    prerequisite = _load_kmpc065(result_dir)
    ...
    checks = {"prerequisite": prerequisite["sha256"] == KMPC065_SHA256,
