"""Bounded runner for KMPC-050 rank-filtered NID provenance capture.
...
CANONICAL_OUTPUT = (
    RESULT_DIR / "RUN_KMPC_050_P5_3G7_NID_ORDER7_M3_PROVENANCE_RANK_FILTER.json"
)
FAILURE_OUTPUT = RESULT_DIR / (
    "RUN_KMPC_050_P5_3G7_NID_ORDER7_M3_PROVENANCE_RANK_FILTER_TECHNICAL_FAILURE.json"
)
...
EXPECTED_PREREQUISITES = {
    ...
    "RUN_KMPC_049_P5_3G7_NID_ORDER7_M3_PROVENANCE_TECHNICAL_FAILURE.json": "EB5EA48145CB52C95826A3111ABBD2DF0C05AC531B8471DF704E2032FA0B6E35",
}
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-050 output conflict: {conflicts}")
    actual = {
        name: _sha256_file(BASE_DIR / name) for name in EXPECTED_SOURCE_HASHES
    }
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(
            f"KMPC-050 exact pre-import source hash mismatch: {different}"
        )
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(
                f"KMPC-050 immutable prerequisite hash mismatch: {name}"
            )
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-050 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC050_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-050 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded KMPC-050 rank-filtered NID order-7 M3 provenance; "
            "no support, equation, threshold, or physics change."
        )
    )
...
def main() -> int:
    ...
    try:
        ...
        if args.audit:
            ...
            requested = (
                args.output if args.output.is_absolute() else SCRIPT_DIR.parent / args.output
            )
            if requested.resolve() != CANONICAL_OUTPUT.resolve():
                raise ValueError("audit requires canonical KMPC-050 --output")
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-050 payload source hash mismatch")
            ...
            json.dumps(
                {
                    ...
                    "provenance_pass": safe["provenance_pass"],
                    "matrix_pass": safe["matrix_provenance_guard"]["pass"],
                    "correction_pattern_pass": safe[
