"""Bounded runner for KMPC-049 NID order-7 M3 provenance.
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_049_P5_3G7_NID_ORDER7_M3_PROVENANCE.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_049_P5_3G7_NID_ORDER7_M3_PROVENANCE_TECHNICAL_FAILURE.json"
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-049 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-049 exact pre-import source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-049 immutable prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-049 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC049_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-049 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded KMPC-049 NID order-7 M3 capture-only provenance and "
            "same-matrix diagnostic correction; no support or physics change."
        )
    )
...
def main() -> int:
    ...
    try:
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-049 payload source hash mismatch")
            ...
            json.dumps(
                {
                    ...
                    "correction_pattern_pass": safe["same_matrix_correction_diagnostic"]["pass_numerical_boundary_pattern"],
