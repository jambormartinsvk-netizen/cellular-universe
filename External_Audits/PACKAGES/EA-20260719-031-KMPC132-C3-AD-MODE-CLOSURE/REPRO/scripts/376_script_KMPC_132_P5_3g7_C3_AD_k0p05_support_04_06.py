"""CLI runner for KMPC-132 six-shard AD/.05 C3 support successor.
...
from baseScripts.p5_general_synchronous import (
    c3_zero_variant_parallel_v4_ad_k0p05_support_04_06 as c3,
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run AD/.05 C3 support 04/06 through six bounded shards."
    )
    ...
    parser.add_argument("--mode", choices=("AD",), required=True)
    parser.add_argument("--k", type=float, choices=(0.05,), required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not _records_pass(records):
            raise RuntimeError("one or more KMPC-132 support workers failed")
        ...
        failure_payload = {
            ...
            "identity": {
                ...
                "physical_receipt": "support_04_06_six_shards",
            },
    ...
    print(json.dumps({
        ...
        "nominal_checkpoint_pass": payload["nominal_support_checkpoint"][
            "checkpoint_pass"
        ],
