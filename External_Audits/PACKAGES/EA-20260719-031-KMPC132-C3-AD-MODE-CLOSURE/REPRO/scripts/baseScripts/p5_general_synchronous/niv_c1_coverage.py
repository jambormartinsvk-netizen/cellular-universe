"""Bounded NIV GLOBAL_C1 primary/extended atom for KMPC-054.

Theory author: Martin Jambor. Script creator: Codex (OpenAI).
No NID coefficients, correction vector, or support verdict are transferred.
"""
...
from . import nid_c1_coverage as nid_reference
...
def source_hashes() -> dict[str, str]:
    ...
    hashes = dict(nid_reference.source_hashes())
    hashes["niv_c1_coverage.py"] = sha256_file(here / "niv_c1_coverage.py")
    return hashes


def make_deadline(limit: float) -> tuple[float, Callable[[], None]]:
    if not math.isfinite(limit) or limit <= 0.0 or limit > 4.8:
        raise ValueError("KMPC-054 runtime must be in (0, 4.8] seconds")
    started = time.monotonic()

    def deadline() -> None:
        if time.monotonic() - started > limit:
            raise TimeoutError("KMPC-054 NIV C1 internal deadline exceeded")
...
def _support_guard() -> dict[str, object]:
    ...
    checks = {
        "contract_primary_exact_minus1_2": tuple(spec["primary"]) == PRIMARY,
        "implementation_primary_equals_contract": implementation_primary == tuple(spec["primary"]),
        "derived_extended_exact_minus1_4": implementation_extended == EXTENDED,
        "derived_extended_equals_contract": implementation_extended == tuple(spec["extended"]),
        "leading_j_exact_minus1": derived["leading_j"] == spec["leading_j"] == LEADING_J,
        ...
        "M1_order5_covers_extended_high4": 5 >= EXTENDED[1],
...
def _pure_added_tail(
    ...
    for z in physics.Z_SURFACES:
        ...
        for name in state_order:
            ...
            states[name] = {
                "base_primary_minus1_2": base,
                "signed_added_tail_34_diagnostic": signed_added,
                "absolute_term_envelope_34_authoritative": envelope,
                "full_extended_minus1_4": full,
    ...
    return {
        ...
        "expected_added_powers": [3, 4],
        "authoritative_metric": "sum(abs(c_j)*z**j) for j=3,4",
        "signed_tail_role": "DIAGNOSTIC_ONLY",
        "by_z": by_z,
        "all_finite": all_finite,
        "pass": bool(all_pass and all_finite and added_powers == (3, 4)),
    }
...
def _combined_rfs_guard(
    ...
    expected_leading = {
        "U_gamma": -3.0 * rfs / (4.0 * rg),
        "U_fs": 0.75,
        "U_b": -3.0 * rfs / (4.0 * rg),
    }
    observed_leading = {
        name: float(standard[name].get(-1, 0.0)) for name in expected_leading
    }
    differences = {
        name: abs(observed_leading[name] - expected) for name, expected in expected_leading.items()
    }
    other_leading_names = tuple(name for name in standard if name not in expected_leading)
    other_leading = {name: float(standard[name].get(-1, 0.0)) for name in other_leading_names}
    required_zero_order_names = (
        "h", "eta", "delta_gamma", "delta_fs", "delta_b", "delta_c", "sigma_fs", "U_c"
    )
    required_zero_order = {
        name: float(standard[name].get(0, 0.0)) for name in required_zero_order_names
    }
    velocity_residual = rg * observed_leading["U_gamma"] + rfs * observed_leading["U_fs"]
    split_residual = rnu + rsteam - rfs
    tolerance = physics.ABS_FALLBACK_TOL
    return {
        "weights": {"R_gamma": rg, "R_fs": rfs, "R_nu": rnu, "R_steam": rsteam},
        "leading_power": -1,
        "observed_leading": observed_leading,
        "expected_combined_R_fs": expected_leading,
        "absolute_differences": differences,
        "other_leading_coefficients_expected_zero": other_leading,
        "required_zero_order_coefficients_expected_zero": required_zero_order,
        "velocity_compensation_residual": velocity_residual,
        "R_nu_plus_R_steam_minus_R_fs": split_residual,
        "velocity_bridge": collective_contract.canonical_candidate()["velocity_bridge"],
        "pass": bool(
            max(differences.values(), default=0.0) <= tolerance
            and max((abs(value) for value in other_leading.values()), default=0.0) <= tolerance
            and max((abs(value) for value in required_zero_order.values()), default=0.0) <= tolerance
            and abs(velocity_residual) <= physics.STEAM_SPLIT_TOL
            and abs(split_residual) <= physics.STEAM_SPLIT_TOL
            and collective_contract.canonical_candidate()["velocity_bridge"]
            == "P5_U_REGISTERED_NOT_SCRIPT84_q"
        ),
    }
...
def run_smoke(max_runtime_seconds: float, result_dir: Path) -> dict[str, object]:
    ...
    wrong_support["mode_spec"][MODE]["extended"] = (-1, 5)
    wrong_weight = collective_contract.canonical_candidate()
    wrong_weight["niv_compensation_weight"] = "R_nu"
    checks = {
        ...
        "wrong_NIV_extended_support_rejected": not collective_contract.validate_candidate(wrong_support).valid,
        "wrong_NIV_R_nu_compensation_rejected": not collective_contract.validate_candidate(wrong_weight).valid,
        "fixed_identity": MODE == "NIV" and K_MPC == 0.05 and VARIANT == "nominal",
        ...
        "immutable_previous_NID_closure": bool(
...
def run_audit(max_runtime_seconds: float, result_dir: Path) -> dict[str, object]:
    ...
    core_checks = {
        ...
        "NIV_combined_R_fs_compensation": bool(rfs_guard["pass"]),
        "NIV_core_gates": bool(core["pass"]),
        "S_C0_actual_primary_extended_lower_moments": bool(s_c0_guard["pass"]),
        "all_common_tail_and_solve_fields_finite": nid_reference._all_finite(
            {"result": result, "common": common, "tails": tails}
        ),
    }
    core_pass = all(core_checks.values())
    if not core_pass:
        candidate = "REVIEW_NIV_C1_CORE_GATE_UNCLOSED"
    elif not common_pass:
        candidate = "REVIEW_NIV_C1_COMMON_COEFFICIENT_CONVERGENCE_UNCLOSED"
    elif not tail_pass:
        candidate = "REVIEW_NIV_C1_SUPPORT_EXTENSION_REQUIRED"
    else:
        candidate = "PASS_NIV_C1_PRIMARY_EXTENDED_ATOM_CANDIDATE_ONLY"
    deadline()
    payload = {
        "test": "A2-K4 P5.3g7 GLOBAL_C1 NIV primary [-1,2] versus extended [-1,4]",
        "run_id": RUN_ID,
        ...
        "scope": {
            "included": "NIV primary [-1,2] versus extended [-1,4], leading j=-1 combined-R_fs compensation, F0/M3 common -1..2 and tail 3,4",
            "excluded": "further NIV support, other k/variants, S-M, full hierarchy, finite opacity, ODE, P5.4, G8/G9, CLASS/CMB/BBN/S8/H0",
        },
        "support_count_guard": support_guard,
        "immutable_prerequisites": {
            "S_C0": {"file": S_C0_RESULT[0], "sha256": passport_hash},
            "previous_NID_closure": {
                "file": PREVIOUS_COVERAGE_RESULT[0],
                "sha256": previous_hash,
                "role": "SEQUENTIAL_ONLY_NO_NUMERICAL_STATE_TRANSFER",
            },
        },
        ...
        "NIV_combined_R_fs_compensation_guard": rfs_guard,
        "core_gate": core,
        "S_C0_actual_coefficient_guard": s_c0_guard,
        "common_coefficient_bridges_minus1_2_to_minus1_4": common,
        "common_coefficient_pass": common_pass,
        "pure_added_tails_34": tails,
    ...
    if not nid_reference._all_finite(payload):
        raise FloatingPointError("non-finite value in final KMPC-054 payload")
    return payload
