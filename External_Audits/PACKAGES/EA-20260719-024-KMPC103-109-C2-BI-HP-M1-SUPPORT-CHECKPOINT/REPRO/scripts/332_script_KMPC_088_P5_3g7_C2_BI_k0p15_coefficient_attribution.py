...
from baseScripts.p5_general_synchronous import c2_bi_k0p15_coefficient_attribution as audit  # noqa: E402

if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-088 deterministic single-thread environment not active")

audit.configure(
    run_id="KMPC-088", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_088_P5_3G7_C2_BI_K0p15_COEFFICIENT_ATTRIBUTION.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-088",
        audit_module="baseScripts.p5_general_synchronous.c2_bi_k0p15_coefficient_attribution",
        aggregate_name="NOT_AVAILABLE_KMPC_088.json",
