"""Checkpoint runner for KMPC-077 C2 BI/k=.005 accepted support [0,7].
...
if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-077 deterministic single-thread environment not active")

audit.configure(
    run_id="KMPC-077", phase="checkpoint", mode="BI", k_mpc=0.005,
    accepted=(0, 7), audit=(0, 9), m1_depth=9,
    output_name="RUN_KMPC_077_P5_3G7_C2_BI_K0p005_SUPPORT_07_ACCEPTED_CHECKPOINT.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        ...
        aggregate_name="NOT_AVAILABLE_KMPC_077.json",
