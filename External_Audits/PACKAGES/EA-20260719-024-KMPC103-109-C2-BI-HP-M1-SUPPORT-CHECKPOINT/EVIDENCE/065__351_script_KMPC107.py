"""Routing successor for the immutable BI/k=.15 HP-M1 checkpoint.
...
from baseScripts.p5_general_synchronous import c2_bi_k0p15_high_precision_m1_reassembly_v14_checkpoint_identity_successor as audit  # noqa: E402

if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-107 deterministic single-thread environment not active")
...
def _load_literal_ancestor_contract() -> tuple[dict[str, str], dict[str, str]]:
    ...
    failed = here / FAILED_RUNNER_NAME
    if harness.stable.sha256_file(failed) != FAILED_RUNNER_SHA256:
        raise RuntimeError("KMPC-107 failed-runner lineage hash mismatch")
    path = here / LITERAL_ANCESTOR_NAME
    if harness.stable.sha256_file(path) != LITERAL_ANCESTOR_SHA256:
        raise RuntimeError("KMPC-107 literal ancestor hash mismatch")
    ...
    for node in tree.body:
        ...
        if not isinstance(node.value, ast.Dict):
            raise TypeError(f"KMPC-107 {target.id} ancestor is not a literal dict")
        value = ast.literal_eval(node.value)
        if not isinstance(value, dict):
            raise TypeError(f"KMPC-107 {target.id} is not a literal dict")
        found[target.id] = value
    if set(found) != wanted:
        raise RuntimeError("KMPC-107 literal ancestor contract incomplete")
...
audit.configure(
    run_id="KMPC-107", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_107_P5_3G7_C2_BI_K0p15_HP_M1_SUPPORT_CHECKPOINT_ROUTING_SUCCESSOR.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-107",
        audit_module="baseScripts.p5_general_synchronous.c2_bi_k0p15_high_precision_m1_reassembly_v14_checkpoint_identity_successor",
        aggregate_name="NOT_AVAILABLE_KMPC_107.json",
