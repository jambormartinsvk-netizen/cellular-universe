"""Read-only receipt runner for the immutable KMPC-108 checkpoint.
...
from baseScripts.p5_general_synchronous import c2_bi_k0p15_high_precision_m1_reassembly_v16_checkpoint_receipt as audit  # noqa: E402

if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-109 deterministic single-thread environment not active")

PUBLISHED_RUNNER_NAME = (
    "352_script_KMPC_108_P5_3g7_C2_BI_k0p15_HP_M1_support_checkpoint_JSON_successor.py"
)
PUBLISHED_RUNNER_SHA256 = (
...
def _load_literal_ancestor_contract() -> tuple[dict[str, str], dict[str, str]]:
    ...
    if harness.sha256_file(here / PUBLISHED_RUNNER_NAME) != PUBLISHED_RUNNER_SHA256:
        raise RuntimeError("KMPC-109 published-runner lineage hash mismatch")
    path = here / LITERAL_ANCESTOR_NAME
    if harness.sha256_file(path) != LITERAL_ANCESTOR_SHA256:
        raise RuntimeError("KMPC-109 literal ancestor hash mismatch")
    ...
    for node in tree.body:
        ...
        if not isinstance(node.value, ast.Dict):
            raise TypeError(f"KMPC-109 {target.id} ancestor is not a literal dict")
        value = ast.literal_eval(node.value)
        if not isinstance(value, dict):
            raise TypeError(f"KMPC-109 {target.id} is not a literal dict")
        found[target.id] = value
    if set(found) != wanted:
        raise RuntimeError("KMPC-109 literal ancestor contract incomplete")
...
audit.configure(
    run_id="KMPC-109", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_109_P5_3G7_C2_BI_K0p15_HP_M1_SUPPORT_CHECKPOINT_READ_ONLY_RECEIPT.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-109",
        audit_module="baseScripts.p5_general_synchronous.c2_bi_k0p15_high_precision_m1_reassembly_v16_checkpoint_receipt",
        aggregate_name="NOT_AVAILABLE_KMPC_109.json",
