...
read-only overili lossless 13-state downstream checkpoint. M1, accepted
`[0,5]`, F0, common, tail, S-C0, background aj audit holdout prešli; jediný
otvorený check je float64 audit `M3_driver=2.7716e-10 > 1e-10` na
`tight_coupling[7]`. C2 je `5/10 PASS`; ďalší krok je jediný exact-driver/
non-fit-holdout resume z hashovaného checkpointu, bez opakovania CPQR.
...
| P5.3 | B1 PASS; `.05/nominal` support uzavretý; C2 `5/10 PASS` | AD/CDI a BI/k=.005 uzavreté; BI/k=.15 lossless checkpoint je receipt-overený, čaká jediný exact M3 driver/non-fit holdout resume |
