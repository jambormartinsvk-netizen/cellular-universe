"""Identity successor for BI/k=.15 HP-M1 downstream insertion.
...
from baseScripts.p5_general_synchronous import c2_bi_k0p15_high_precision_m1_reassembly_v12_downstream_identity_successor as audit  # noqa: E402

if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-105 deterministic single-thread environment not active")
...
def _load_prior_literal_contract() -> tuple[dict[str, str], dict[str, str]]:
    ...
    if harness.stable.sha256_file(path) != PRIOR_RUNNER_SHA256:
        raise RuntimeError("KMPC-105 prior runner contract hash mismatch")
    ...
    for node in tree.body:
        ...
        if isinstance(target, ast.Name) and target.id in wanted:
            ...
            if not isinstance(value, dict):
                raise TypeError(f"KMPC-105 {target.id} is not a literal dict")
            found[target.id] = value
    if set(found) != wanted:
        raise RuntimeError("KMPC-105 prior runner literal contract incomplete")
...
audit.configure(
    run_id="KMPC-105", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_105_P5_3G7_C2_BI_K0p15_HP_M1_DOWNSTREAM_IDENTITY_SUCCESSOR.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-105",
        audit_module="baseScripts.p5_general_synchronous.c2_bi_k0p15_high_precision_m1_reassembly_v12_downstream_identity_successor",
        aggregate_name="NOT_AVAILABLE_KMPC_105.json",
