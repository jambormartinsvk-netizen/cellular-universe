...
## Povinné vyhodnotenie progresu a odklonu od cieľa

- Po každej uzavretej úlohe alebo výpočtovom atóme, ktorý zmenil dôkaz,
  blocker, aktívny ďalší krok alebo spotreboval technický pokus, nasleduje
  read-only posudok roly `progress_goal_reviewer` ešte pred otvorením ďalšej
  vedeckej úlohy. Nejde o audit každého shell príkazu ani nedokončeného
  vnútorného medzikroku.
- Posudok porovná deklarovaný cieľ a `DONE_WHEN` so skutočnými immutable
  výstupmi, uvedie autoritatívny stav pred/po, dotknutý gate a hĺbku, nový
  informačný zisk, náklady v súboroch/auditoch/výpočtoch, prínos na ceste k
  A3, duplicity, odklon od cieľa a najmenší užitočný successor.
- Prvý posudok route smie vytvoriť baseline. Ďalšie posudky načítajú povinný
  bootstrap, aktívny kapsul, autoritatívny stav a iba exact delta artefakty.
  Byte-identické package kópie sa pri preukázanej manifest parity znovu celé
  nečítajú a reviewer nerobí rekurzívny scan repozitára; pri nedostatku dôkazu
  vráti presný blocker.
- Povolené primárne klasifikácie sú
  `SCIENTIFIC_GATE_PROGRESS`, `BOUNDARY_OR_BLOCKER_PROGRESS`,
  `TECHNICAL_ENABLEMENT_ONLY`, `DOCUMENTATION_OR_AUDIT_CLOSURE_ONLY`,
  `NO_INFORMATIONAL_PROGRESS` a `GOAL_DRIFT`.
- Počet súborov, skriptov, testov alebo auditov sám nezvyšuje vedecké skóre.
  Technická či dokumentačná práca môže byť potrebná, ale nesmie sa vydávať za
  nový fyzikálny dôkaz.
- Rola je poradná a read-only. Nemení `PASS/REVIEW/STOP`, skóre, hĺbku,
  `RUN_AUTHORIZED`, fyzikálny scope ani registre; prípadné prijaté zmeny
  vykoná iba hlavný orchestrátor.
- Pri `GOAL_DRIFT_ALERT` sa pred otvorením novej vetvy určí presný bod
  odklonu, posledný platný uzol route a návratový krok. Výnimkou sú iba
  vysvetľujúce/read-only otázky bez nového artefaktu alebo zmeny stavu.
