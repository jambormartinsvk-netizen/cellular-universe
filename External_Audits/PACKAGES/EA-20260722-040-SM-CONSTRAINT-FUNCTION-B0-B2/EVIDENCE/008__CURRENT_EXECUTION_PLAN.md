...
| P5.3 | B1 PASS; C2 `10/10` a aggregate PASS; C3 autoritatívne `45/45` logical PASS aj aggregate PASS; S-M `D01 CLOSED V1-R1 / D02 CLOSED_FORM_ONLY V1-S1+V1-P1 / D03 ACTIVE` | AD, CDI, BI, NID aj NIV sú `9/9`; `V_min=0`, ale `m_e` a počiatočné dáta sú D06; transfer/frame/clock ešte nie sú zvolené |
...
1. C3 kontrakt aj KMPC-148 aggregate sú uzavreté na `45/45 logical PASS`.
   ...
   patria D06. Jediný aktívny krok je teraz `V1-D03`. Constraint-first mapa
   ...
   Najbližší krok D03a je odvodiť konečnú sadu bunkových kandidátov
   `R_J(Y),E_J(Y)` z geometrie delenia a `delta`, nie ich fitovať.
