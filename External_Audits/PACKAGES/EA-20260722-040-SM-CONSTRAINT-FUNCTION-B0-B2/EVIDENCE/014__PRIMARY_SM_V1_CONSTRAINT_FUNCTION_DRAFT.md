...
V rámci B2 diskrétnej eventovej vetvy najbližší krok nie je hľadať ďalší
tvar `beta_s`. Je ním constraint-first odvodenie konečnej sady bunkových kandidátov pre
`E_J(Y)` a `R_J(Y)` z geometrie delenia, energetickej réžie `delta` a
existujúcej event/failure filozofie. Až ich súčin smie byť spätne porovnaný
s makroskopickým parent drainom. Relaxačný alebo iný prompt operator zostáva
legitímnou samostatnou možnosťou z B1.
