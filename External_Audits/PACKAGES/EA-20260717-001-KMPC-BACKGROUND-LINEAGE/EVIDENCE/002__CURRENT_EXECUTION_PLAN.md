...
## 0. Týždenná tokenová pauza

**Stav:** `PAUSED_HIGH_COST_PHYSICS_UNTIL_BUDGET_REFRESH`  
**Predpokladaný návrat:** 2026-07-24  
**Dôvod:** spotrebovaných viac než približne 65 % týždenného tokenového
rozpočtu; správnosť má prednosť pred urýchleným pokračovaním lacnejším
reasoningom na fyzikálnom verdikte.

Počas pauzy sa nesmie spustiť KMPC-037, high-precision solve, nový fyzikálny
variant ani meniť PASS/REVIEW/STOP. Povolené sú iba lacné mechanické práce:
hash/link kontrola, inventár duplicít, návrh konsolidácie dokumentácie a
read-only triedenie artefaktov. Tie môže vykonávať rýchlejší model, ale nesmie
meniť vzorce ani autoritatívny stav.

Presný bod návratu: predregistrovať
`M1_ORDER7_NUMERICAL_REFINEMENT_AND_BOUNDARY_CLOSURE_AUDIT` na nezmenenej
KMPC-036 sústave a prahoch. Support step 3 zostáva `BLOCKED`. Aktuálny dôkaz:
KMPC-036 SHA `39BB3886...B7B497`, tri terminal driver `[7]` precision REVIEW,
K4 `LIVE / 60/100`, active technical counter `0/10`.
...
| P5.3 | B1 PASS; AD sentinel PASS; CDI step-2 core/common PASS; M1 order-7 provenance/holdout scoped PASS | KMPC-036: tri terminal driver `[7]` precision REVIEW; ďalší krok precision/boundary closure audit; support step 3 BLOCKED |
...
1. kontrakt S1/doc51 a KMPC-033 lower-moment S-C0 passport sú scoped PASS;
   KMPC-035 CDI step 2 dal core/common PASS, ale `[0,3]` remainder REVIEW;
   KMPC-036 skončil scoped PASS + terminal precision REVIEW; najprv vykonať
   M1 order-7 precision/boundary closure audit a iba po jeho hlavnom PASS
   support step 3 `[0,5]→[0,7]`; potom fail-fast BI, NID a NIV pri `k=.05`,
   nominal, každý s vlastným leading power a primary/extended supportom;
