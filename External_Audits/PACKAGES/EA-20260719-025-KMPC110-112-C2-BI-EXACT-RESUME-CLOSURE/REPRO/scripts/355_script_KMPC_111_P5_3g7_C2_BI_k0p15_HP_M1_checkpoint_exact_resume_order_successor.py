"""Order-reconstruction successor for checkpointed exact M3 resume.
...
from baseScripts.p5_general_synchronous import c2_bi_k0p15_high_precision_m1_reassembly_v18_exact_resume_order_successor as audit  # noqa: E402

if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-111 deterministic single-thread environment not active")
...
def _load_literal_ancestor_contract() -> tuple[dict[str, str], dict[str, str]]:
    ...
    if harness.stable.sha256_file(here / PRIOR_RUNNER_NAME) != PRIOR_RUNNER_SHA256:
        raise RuntimeError("KMPC-111 prior-runner lineage hash mismatch")
    path = here / LITERAL_ANCESTOR_NAME
    if harness.stable.sha256_file(path) != LITERAL_ANCESTOR_SHA256:
        raise RuntimeError("KMPC-111 literal ancestor hash mismatch")
    ...
    for node in tree.body:
        ...
        if not isinstance(node.value, ast.Dict):
            raise TypeError(f"KMPC-111 {target.id} ancestor is not a literal dict")
        value = ast.literal_eval(node.value)
        if not isinstance(value, dict):
            raise TypeError(f"KMPC-111 {target.id} is not a literal dict")
        found[target.id] = value
    if set(found) != wanted:
        raise RuntimeError("KMPC-111 literal ancestor contract incomplete")
...
audit.configure(
    run_id="KMPC-111", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_111_P5_3G7_C2_BI_K0p15_HP_M1_CHECKPOINT_EXACT_RESUME_ORDER_SUCCESSOR.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-111",
        audit_module="baseScripts.p5_general_synchronous.c2_bi_k0p15_high_precision_m1_reassembly_v18_exact_resume_order_successor",
        aggregate_name="NOT_AVAILABLE_KMPC_111.json",
