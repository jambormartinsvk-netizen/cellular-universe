...
**Stav:** `ŽIVÁ / active technical counter 0/10 / .05 nominal support PASS / C2 6/10 PASS — AD/CDI/BI oba k uzavreté; NID/NIV, S-M, hierarchy a zvyšná coverage otvorené`  
...
| P5.3 | regulárne general-synchronous seedy v plnom stave | **ČIASTOČNÝ PASS / C2 6/10 PASS** | AD/CDI/BI oba k uzavreté; ďalší je NID/`.005`, potom NID/`.15` a oba NIV body; štyri atómy NOT_RUN. |
...
hashovaný checkpoint. KMPC-080 opravil BI/`.15` main driver, ale nezávislý
  ...
  BI/.15 scoped PASS. C2 je
  `6/10 PASS`; štyri ďalšie atómy čakajú. Ďalší je NID/`.005` s frozen
  `[0,5]→[0,7]`. S-C zostáva conditional a fyzická S-M para otvorená.
