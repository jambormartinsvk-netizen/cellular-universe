"""Publish-canonical parity successor for checkpointed exact M3 resume.
...
from baseScripts.p5_general_synchronous import c2_bi_k0p15_high_precision_m1_reassembly_v19_exact_resume_json_parity_successor as audit  # noqa: E402

if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-112 deterministic single-thread environment not active")
...
def _load_literal_ancestor_contract() -> tuple[dict[str, str], dict[str, str]]:
    ...
    if harness.stable.sha256_file(here / PRIOR_RUNNER_NAME) != PRIOR_RUNNER_SHA256:
        raise RuntimeError("KMPC-112 prior-runner lineage hash mismatch")
    path = here / LITERAL_ANCESTOR_NAME
    if harness.stable.sha256_file(path) != LITERAL_ANCESTOR_SHA256:
        raise RuntimeError("KMPC-112 literal ancestor hash mismatch")
    ...
    for node in tree.body:
        ...
        if not isinstance(node.value, ast.Dict):
            raise TypeError(f"KMPC-112 {target.id} ancestor is not a literal dict")
        value = ast.literal_eval(node.value)
        if not isinstance(value, dict):
            raise TypeError(f"KMPC-112 {target.id} is not a literal dict")
        found[target.id] = value
    if set(found) != wanted:
        raise RuntimeError("KMPC-112 literal ancestor contract incomplete")
...
EXPECTED_SOURCE_HASHES.update({
    ...
    "c2_bi_k0p15_high_precision_m1_reassembly_v18_exact_resume_order_successor.py":
        "3E30375748130FC69D51C2715D6521D3E1637E8F4C548E5605DCD3AB086AB492",
    "c2_bi_k0p15_high_precision_m1_reassembly_v19_exact_resume_json_parity_successor.py":
...
EXPECTED_PREREQUISITES.update({
    ...
    "RUN_KMPC_111_P5_3G7_C2_BI_K0p15_HP_M1_CHECKPOINT_EXACT_RESUME_ORDER_SUCCESSOR_TECHNICAL_FAILURE.json":
        "1ADCB30A3A41AC38EF29E454016AE164A05CC8DD489D6F254B26A91C3BD95E40",
})

audit.configure(
    run_id="KMPC-112", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_112_P5_3G7_C2_BI_K0p15_HP_M1_CHECKPOINT_EXACT_RESUME_JSON_PARITY_SUCCESSOR.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-112",
        audit_module="baseScripts.p5_general_synchronous.c2_bi_k0p15_high_precision_m1_reassembly_v19_exact_resume_json_parity_successor",
        aggregate_name="NOT_AVAILABLE_KMPC_112.json",
