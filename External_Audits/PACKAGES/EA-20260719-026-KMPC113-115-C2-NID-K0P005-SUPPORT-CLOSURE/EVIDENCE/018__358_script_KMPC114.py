"""Checkpoint runner for KMPC-114 C2 NID/k=.005 accepted support [0,7].
...
if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-114 deterministic single-thread environment not active")

audit.configure(
    run_id="KMPC-114", phase="checkpoint", mode="NID", k_mpc=0.005,
    accepted=(0, 7), audit=(0, 9), m1_depth=9,
    output_name="RUN_KMPC_114_P5_3G7_C2_NID_K0p005_SUPPORT_07_ACCEPTED_CHECKPOINT.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        ...
        aggregate_name="NOT_AVAILABLE_KMPC_114.json",
