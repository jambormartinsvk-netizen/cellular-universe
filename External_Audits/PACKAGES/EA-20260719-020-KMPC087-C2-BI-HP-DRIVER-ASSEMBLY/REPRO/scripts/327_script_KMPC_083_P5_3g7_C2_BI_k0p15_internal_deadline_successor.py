"""Internal-deadline successor for KMPC-083 BI/k=.15 80-dps boundary.
...
from baseScripts.p5_general_synchronous import c2_bi_k0p15_high_precision_holdout_v2_deadline as audit  # noqa: E402

if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-083 deterministic single-thread environment not active")

audit.configure(
    run_id="KMPC-083", mode="BI", k_mpc=0.15,
    output_name="RUN_KMPC_083_P5_3G7_C2_BI_K0p15_HIGH_PRECISION_HOLDOUT_BOUNDARY.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-083",
        audit_module="baseScripts.p5_general_synchronous.c2_bi_k0p15_high_precision_holdout_v2_deadline",
        aggregate_name="NOT_AVAILABLE_KMPC_083.json",
