"""Bounded runner for KMPC-047 GLOBAL_C1 / NID primary-extended coverage.
...
CANONICAL_OUTPUT = RESULT_DIR / "RUN_KMPC_047_P5_3G7_NID_C1_PRIMARY_EXTENDED_COVERAGE.json"
FAILURE_OUTPUT = RESULT_DIR / "RUN_KMPC_047_P5_3G7_NID_C1_PRIMARY_EXTENDED_COVERAGE_TECHNICAL_FAILURE.json"
...
EXPECTED_PREREQUISITES = {
    "RUN_KMPC_033_P5_3G7_S_C0_COEFFICIENT_PASSPORT_RERUN1.json": "4CED9D48FD9866113739580E20F69E8122D70204E37C055251C8A49B3E0CFE8C",
...
def _preimport_guard() -> dict[str, str]:
    ...
    if conflicts:
        raise FileExistsError(f"immutable KMPC-047 output conflict: {conflicts}")
    ...
    if actual != EXPECTED_SOURCE_HASHES:
        ...
        raise RuntimeError(f"KMPC-047 exact pre-import source hash mismatch: {different}")
    for name, expected in EXPECTED_PREREQUISITES.items():
        if _sha256_file(RESULT_DIR / name) != expected:
            raise RuntimeError(f"KMPC-047 immutable prerequisite hash mismatch: {name}")
    return actual
...
def _runtime_guard(value: float | None) -> None:
    if value is None or not math.isfinite(value) or value != 4.8:
        raise ValueError("KMPC-047 requires exactly 4.8 runtime seconds")


def _runner_fixtures() -> dict[str, bool]:
    fixture = RESULT_DIR / ".KMPC047_publish_collision_fixture.json"
    temporary = fixture.parent / f".tmp-{fixture.name}"
    if fixture.exists() or temporary.exists():
        raise FileExistsError("stale KMPC-047 publish fixture")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Bounded KMPC-047 GLOBAL_C1 NID atom: primary [0,3], extended "
            "[0,5], leading j=0, combined-R_fs compensation."
        )
    )
...
def main() -> int:
    ...
    try:
        ...
        if payload["source_hashes"] != preimport:
            raise RuntimeError("KMPC-047 payload source hash mismatch")
            ...
            json.dumps(
                {
                    ...
                    "combined_R_fs_pass": safe["NID_combined_R_fs_compensation_guard"]["pass"],
