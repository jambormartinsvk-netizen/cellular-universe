"""Widened-support same-matrix runner for KMPC-125 C2 NIV/k=.15.
...
from baseScripts.p5_general_synchronous import c2_cdi_k0p15_same_matrix_refinement as audit  # noqa: E402


if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-125 deterministic single-thread environment not active")

audit.configure(
    run_id="KMPC-125", mode="NIV", k_mpc=0.15,
    output_name=(
        "RUN_KMPC_125_P5_3G7_C2_NIV_K0p15_SUPPORT_06_08_"
        "SAME_MATRIX_REFINEMENT.json"
    ),
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        run_id="KMPC-125",
        audit_module="baseScripts.p5_general_synchronous.c2_cdi_k0p15_same_matrix_refinement",
        aggregate_name="NOT_AVAILABLE_KMPC_125.json",
