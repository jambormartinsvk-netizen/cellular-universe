"""Checkpoint runner for KMPC-123 C2 NIV/k=.15 accepted [-1,6].
...
if any(os.environ.get(name) != value for name, value in THREAD_ENV.items()):
    raise RuntimeError("KMPC-123 deterministic single-thread environment not active")

audit.configure(
    run_id="KMPC-123", phase="checkpoint", mode="NIV", k_mpc=0.15,
    accepted=(-1, 6), audit=(-1, 8), m1_depth=8,
    output_name="RUN_KMPC_123_P5_3G7_C2_NIV_K0p15_SUPPORT_06_ACCEPTED_CHECKPOINT.json",
...
if __name__ == "__main__":
    raise SystemExit(harness.run_cli(
        ...
        aggregate_name="NOT_AVAILABLE_KMPC_123.json",
