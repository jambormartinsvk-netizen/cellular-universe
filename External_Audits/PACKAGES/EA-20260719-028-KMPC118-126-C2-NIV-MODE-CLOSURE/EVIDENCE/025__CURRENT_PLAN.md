...
`2.81e-12/3.40e-12`. Interný audit 197 uzavrel NIV mód. C2 má `10/10`
scoped PASS atómov; povinný read-only aggregate ešte nebežal.
...
| P5.3 | B1 PASS; `.05/nominal` support uzavretý; C2 atómy `10/10 PASS`, aggregate NOT_RUN | všetkých päť módov na oboch k je scoped PASS; aggregate musí overiť kartézsky register a background spread pred C3 |
...
1. všetkých desať C2 mode×k atómov má scoped PASS; po zapečatení spoločného
   NIV balíka EA-028 je ďalší krok predregistrovaný read-only C2 aggregate,
   ktorý overí exact kartézsky register, hashe, identity, všetky brány a
   cross-mode/cross-k background spread bez nového solve;
